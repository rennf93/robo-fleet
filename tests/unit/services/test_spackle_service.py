...
Mirrors test_pest_control_service.py exactly, minus the theme goal (gap-fill
audits have no cycle-level goal) and with the project-participation gate
matching spackle's own opt-in polarity.
"""
...
from robofleet.services.spackle_service import (
    SpackleService,
    get_spackle_service,
)
...
async def _seed_cycle(
    ...
    markers.set_gap_fill(task, {"items": items})
    await session.flush()
    return task


def _svc(session: AsyncSession) -> SpackleService:
    return get_spackle_service(session)
...
async def test_approve_is_idempotent(db_session: AsyncSession) -> None:
    ...
    result = await db_session.execute(
        select(TaskTable).where(
            TaskTable.source == SPACKLE_ITEM_SOURCE, TaskTable.title == "Item 0"
        )
    )
...
async def test_approve_unopted_project_is_invalid_state(
    ...
    """A project that never opted into spackle refuses materialize-side,
    even if propose_gap_fill's own point-in-time check somehow let the item
...
# board_program_cycles row for "spackle".
# --------------------------------------------------------------------------- #
