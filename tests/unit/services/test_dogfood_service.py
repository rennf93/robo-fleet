...
from robofleet.services.dogfood_service import (
    DogfoodService,
    get_dogfood_service,
)
...
def _item(idx: int, *, status: str = "proposed", project_slug: str) -> dict:
    return {
        ...
        "evidence": (
            ...
            "the write (checked network tab: 200 OK, but no UI feedback)"
        ),
        "status": status,
        "reject_reason": None,
...
async def _seed_cycle(
    ...
    task = TaskTable(
        ...
        acceptance_criteria=["propose_friction_fixes() called once"],
        status=TS.PENDING,
        priority=2,
    ...
    markers.set_friction_fixes(task, {"items": items})
    await session.flush()
    return task


def _svc(session: AsyncSession) -> DogfoodService:
    return get_dogfood_service(session)
...
async def test_approve_is_idempotent(db_session: AsyncSession) -> None:
    ...
    result = await db_session.execute(
        select(TaskTable).where(
            TaskTable.source == DOGFOOD_ITEM_SOURCE, TaskTable.title == "Item 0"
        )
    )
...
async def test_approve_unopted_project_is_invalid_state(
    ...
    """A project that never opted into dogfood refuses materialize-side,
    even if propose_friction_fixes's own point-in-time check somehow let the
    item through (e.g. the opt-in was revoked AFTER the PO proposed it)."""
...
async def test_list_open_cycles_excludes_completed(db_session: AsyncSession) -> None:
    ...
    task = await _seed_cycle(
        db_session,
        items=[_item(0, project_slug="frontend-app")],
...
# board_program_cycles row for "dogfood".
# --------------------------------------------------------------------------- #
    ...
    row = (
        await db_session.execute(
            select(BoardProgramCycleTable).where(
                BoardProgramCycleTable.program_key == "dogfood"
            )
        )
    ).scalar_one()
