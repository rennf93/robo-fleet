...
from robofleet.models.a2a import A2AConversationStatus, A2AMessageKind
...
class TaskTable(Base):
    ...
    # ``_pm_respawn_should_gate`` in robofleet/runtime/orchestrator.py) so the
...
class ProjectTable(Base):
    ...
    # robofleet.services.env_branches synthesizes a degenerate single-branch
    ...
    # short-lived installation token (robofleet/services/github_app_auth.py)
...
class SystemSettingTable(Base):
    ...
    ``robofleet.config`` are the fallback when a key has no row yet.
    """
...
class AgentSpawnSessionTable(Base):
    ...
    # 081_doctrine_version) — lets the eval harness (robofleet/eval/) group spawn
...
class UserTable(Base):
    ...
    (``robofleet.api.auth.seed.ensure_seed_user``).
