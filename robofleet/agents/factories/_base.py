...
def _autogen_verbs_layer(prompts_path: Path, role: "AgentRole") -> str | None:
    ...
    ``robofleet/api/schemas/v1/`` plus the role-config. Including it as a
...
def _lifecycle_layer(prompts_path: Path, role: "AgentRole") -> str | None:
    ...
    ``robofleet/lifecycle/spec.py`` by ``scripts/build_lifecycle_artifacts.py``
...
def compose_prompt(
    ...
    ``robofleet/lifecycle/spec.py`` by ``make lifecycle`` and gated on CI.
