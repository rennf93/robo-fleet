...
from robofleet.api.schemas.common import ErrorCode
from robofleet.config import settings
from robofleet.db.base import get_session_factory
from robofleet.db.tables import VerbLatencySampleTable
...
from robofleet.services.exceptions import RateLimitError
...
async def service_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """Handle `robofleet.services.base.ServiceError` and subclasses."""
...
async def rate_limit_exception_handler(
    ...
    """Handle :class:`~robofleet.services.exceptions.RateLimitError`.
...
class FlowVerbTimeoutMiddleware:
    ...
    ``_SLOW_VERBS`` (from ``robofleet.foundation.policy.flow_timeouts`` — git
    ...
    (``robofleet/mcp/flow_server.py``) so the two walls can't drift apart.
    """
...
class DbCommitMiddleware:
    ...
    ``get_db_committed`` (``robofleet/db/base.py``; the ``robofleet.api.deps.DbSession``
