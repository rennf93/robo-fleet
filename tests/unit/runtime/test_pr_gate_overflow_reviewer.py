...
reviewer. cell-pr-reviewer-2 (board-team, base-image-identical) is the
second candidate in _select_agent_for_cell's pr_reviewer branch, so the
dispatcher falls back to it when the dedicated reviewer is active. It never
serves root->master or inbound external PRs (both hardcode pr-reviewer-1),
preserving the run-order starvation guard.
"""
...
from robofleet.seeds.initial_data import _AGENT_PRESENTATION
...
def test_cell_pr_reviewer_2_image_matches_cell_reviewers() -> None:
    # Delivery reviewers (dedicated + overflow) share the base image: robo-fleet
    # stripped the specialized delivery images, so a delivery slug falls through
    # to AGENT_BASE_IMAGE via get_agent_image's .get default. The overflow
    # reviewer must resolve to the SAME image the cell's own reviewer uses.
    assert "cell-pr-reviewer-2" not in AGENT_IMAGES
    assert get_agent_image("cell-pr-reviewer-2") == get_agent_image("be-pr-reviewer")
