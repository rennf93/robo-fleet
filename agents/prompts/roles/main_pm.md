...
| `robofleet_git_status(project_slug)` / `robofleet_git_log(project_slug, limit?, branch?)` / `robofleet_git_diff(project_slug, branch?, base?)` / `robofleet_git_branches(project_slug)` | Read-only git inspection. Use these (not raw `Bash git ...`) when verifying a cell-PM subtask before completing/merging. | None. |
