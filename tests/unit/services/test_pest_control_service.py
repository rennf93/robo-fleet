"""PestControlService coverage: per-item approve materializes a BACKLOG task
(idempotent), reject records a reason (idempotent), and the exploration task
completes once every item is terminal.

Mirrors test_roadmap_service.py exactly, minus the theme goal (pest hunts
have no cycle-level goal) and with the project-participation gate flipped to
its opt-in polarity.
"""
...
from robofleet.services.pest_control_service import (
    PestControlService,
...
def _item(idx: int, *, status: str = "proposed", project_slug: str) -> dict:
    return {
        ...
        "team": "backend",
        "priority": 2,
        "evidence": f"task_review_findings row F-abc{idx} — waived 3x on this file",
        "status": status,
        "reject_reason": None,
...
async def _seed_cycle(
    ...
    markers.set_pest_hunt(task, {"items": items})
    await session.flush()
    return task


def _svc(session: AsyncSession) -> PestControlService:
    return get_pest_control_service(session)
...
async def test_approve_is_idempotent(db_session: AsyncSession) -> None:
    ...
    result = await db_session.execute(
        select(TaskTable).where(
            TaskTable.source == PEST_CONTROL_ITEM_SOURCE, TaskTable.title == "Item 0"
        )
    )
...
async def test_reject_records_reason(db_session: AsyncSession) -> None:
    ...
    result = await _svc(db_session).reject_item(
        _id(task), "item-0", "already fixed elsewhere"
    )
    assert result is not None
    ...
    assert item0["reject_reason"] == "already fixed elsewhere"
...
async def test_approve_unopted_project_is_invalid_state(
    ...
    """A project that never opted into pest_control refuses materialize-side,
    even if propose_bug_hunt's own point-in-time check somehow let the item
    through (e.g. the opt-in was revoked AFTER the PO proposed it)."""
...
# board_program_cycles row for "pest_control".
# --------------------------------------------------------------------------- #
