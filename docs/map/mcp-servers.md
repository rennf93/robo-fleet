# RoboFleet Slice Map — `mcp-servers`

Scope: `robofleet/mcp/` (every server file + `schemas/` + `utils.py`). Repo root: `/Users/renzof/Documents/GitHub/ZZZ/robofleet-master/robofleet`.

## Purpose

The `robofleet/mcp` package is the agent-side MCP gateway: a set of `FastMCP` server processes that run **inside each agent container** and expose the RoboFleet intent-verb / content-tool / RAG / docs / git-readonly / intake / secretary / web-research surfaces to the Claude Code or grok-CLI runtime as MCP tools. They are thin bridges — every tool either POSTs to the orchestrator's HTTP gateway (`/api/v1/flow/*`, `/api/v1/do/*`, `/api/git/*`, `/optimal/*`, `/docs/*`, `/research/*`, `/api/secretary/*`, `/api/prompter/live/*`) or, for the flow/do path, additionally forwards rejections to a per-container SDK loopback (`ROBOFLEET_SDK_URL`) that runs the per-verb circuit breaker. The orchestrator (not the MCP layer) is the authority for role scoping, state transitions, and git-side effects; the MCP layer only shapes calls, classifies rejections, and substitutes `circuit_open` envelopes when the breaker trips.

## Files

| Path | Role | approx LOC |
|------|------|------------|
| `robofleet/mcp/__init__.py` | Package docstring only — deliberately import-free so `python -m robofleet.mcp.<name>` does not pull sibling modules (esp. `optimal_server`'s pgvector/ollama stack, ~6s startup). | 23 |
| `robofleet/mcp/utils.py` | Shared HTTP helpers: `_get_agent_headers`, `format_error_response`, `ApiResponse`, `ApiClient` (async httpx wrapper). Used by `optimal_server`, `docs_server`, `search_server`. | 383 |
| `robofleet/mcp/schemas/__init__.py` | Pydantic input models. After Phase-4 T9 deletions only `WriteDocInput` remains. | 35 |
| `robofleet/mcp/flow_server.py` | `robofleet-flow` MCP server — intent verbs (lifecycle). Manifest-scoped registration, role-scoped path `/api/v1/flow/<route>/<verb>`, per-verb circuit breaker + 404-route synthesis. | 1028 |
| `robofleet/mcp/do_server.py` | `robofleet-do` MCP server — content tools (commit, note, pitch, propose_roadmap, dm, notify, evidence, progress, playbook curation, pr_update). Manifest-scoped; fixed `/api/v1/do/<verb>` path; mirror circuit breaker. | 976 |
| `robofleet/mcp/optimal_server.py` | `robofleet-optimal` MCP server — RAG / KB / mentor / error / decision / standards / learnings / index-mgmt / proactive-context. Factory `create_optimal_mcp_server(agent_id)`; calls `/optimal/*`. | 1102 |
| `robofleet/mcp/docs_server.py` | `robofleet-docs` MCP server — docs write/read/list/delete via `/docs/*`. Factory `create_docs_mcp_server(agent_id)`. RAG-based dedup on write. | 251 |
| `robofleet/services/docs.py` | `DocsService` — the `/docs/*` route handlers' backing service; refuses `write_doc(doc_type="user_facing")` with a message naming the deployer's docs-site project/URL | 785 |
| `robofleet/mcp/git_readonly.py` | `robofleet-git-readonly` MCP server — four read-only git views (status/log/diff/branch list) via `/api/git/*`. No breaker, no manifest. | 123 |
| `robofleet/mcp/intake_server.py` | `robofleet-intake` MCP server — grok intake path only: `propose_draft` / `propose_batch` POST directly to the prompter-live relay. | 215 |
| `robofleet/mcp/secretary_server.py` | `robofleet-secretary` MCP server — grok secretary path only: `read_company_state` / `read_task` / `submit_directive`, delegating to `agent_sdk.secretary_driver`. | 64 |
| `robofleet/mcp/search_server.py` | `robofleet-search` MCP server — `web_search` / `web_fetch` via `/research/*` (provider key stays server-side). Factory `create_search_mcp_server(agent_id)`. | 130 |
...
|------|------|-----------|----------------|
| `mcp` (flow) | `FastMCP` | `flow_server.py:252` | Server instance `robofleet-flow`; tools registered onto it at import time. |
| `mcp` (do) | `FastMCP` | `do_server.py:220` | Server instance `robofleet-do`. |
| `mcp` (git-readonly) | `FastMCP` | `git_readonly.py:31` | Server instance `robofleet-git-readonly`. |
| `mcp` (intake) | `FastMCP` | `intake_server.py:32` | Server instance `robofleet-intake`. |
...
| `_register_tools` (flow) | func | `flow_server.py:965` | Raise `RuntimeError` if manifest missing and `ROBOFLEET_ALLOW_FULL_TOOLSET` not set; if set, registers full tool set as dev/test escape hatch. |
...
| `_register_tools` (do) | func | `do_server.py:896` | Manifest-scoped registration; raise `RuntimeError` if manifest missing (unless `ROBOFLEET_ALLOW_FULL_TOOLSET` set). |
...
| `create_optimal_mcp_server` | factory | `optimal_server.py:1068` | Build `robofleet-optimal-{agent_id}` server; registers 8 tool groups. |
| `robofleet_kb_search` / `robofleet_rag_query` / `robofleet_kb_stats` | tools | `optimal_server.py:70–213` | Search / RAG / stats. |
| `robofleet_ask_mentor` | tool | `optimal_server.py:371` | Primary conversational RAG tool (65s timeout). |
| `robofleet_search_error` / `robofleet_record_error_solution` | tools | `optimal_server.py:439–535` | Error-pattern memory. |
| `robofleet_check_decision` / `robofleet_record_decision` | tools | `optimal_server.py:542–625` | Decision memory (`RecordDecisionInput` pydantic model at L33). |
| `robofleet_get_standards` / `robofleet_validate_action` / `robofleet_review_code` | tools | `optimal_server.py:632–765` | Standards + validation. |
| `robofleet_record_learning` / `robofleet_search_learnings` | tools | `optimal_server.py:772–875` | Learnings. |
| `robofleet_clear_index` / `robofleet_reindex_all` / `robofleet_index_status` | tools | `optimal_server.py:882–993` | Index admin. |
| `robofleet_get_proactive_context` | tool | `optimal_server.py:1000` | Stored-then-fresh proactive context for a task. |
| `normalize_index_types` | func | `optimal_server.py:53` | Map legacy `docs` alias → `documentation` before route's `IndexType(...)` conversion. |
| `create_docs_mcp_server` | factory | `docs_server.py:160` | Build `robofleet-docs-{agent_id}` server (write/read/list/delete). |
...
| `_refused_doc_types` | func | `services/docs.py:84` | Builds the refusal-message dict fresh on every call from `settings.docs_site_project_slug`/`docs_site_public_url` (was a fixed `REFUSED_DOC_TYPES` module constant hardcoding "robo-fleet-website"/"docs.robo-fleet.tech"); falls back to generic "your docs-site project" / "ship on your docs site" text when either setting is left blank |
| `WriteDocInput` | pydantic | `schemas/__init__.py:12` | Docs write input (task_id, filename, doc_type, title, content). |
| `robofleet_git_status` / `robofleet_git_log` / `robofleet_git_diff` / `robofleet_git_branch_list` | tools | `git_readonly.py:45–119` | Read-only git views via `/api/git/*`. |
...
| `_get_agent_headers` | func | `utils.py:27` | `X-Agent-ID`/`X-Agent-Role`/`X-Agent-Team`/`X-Agent-Token` (HMAC token from `ROBOFLEET_AGENT_TOKEN`). |
| `format_error_response` | func | `utils.py:52` | Wraps `robofleet.api.schemas.common.error_response`. |
| `create_search_mcp_server` | factory | `search_server.py:83` | `robofleet-search-{agent_id}` with `web_search`/`web_fetch`. |

## Data Flow

Every server is launched as its own subprocess (`uv run --no-sync python -m robofleet.mcp.<name> [agent_id]`) by the orchestrator's `_generate_mcp_config` (for flow/do/git-readonly/optimal/docs/search) or by the grok intake/secretary mains (for intake/secretary). At import time the flow/do servers read `/app/tool-manifest.json` (env `ROBOFLEET_TOOL_MANIFEST_PATH`) and register only the verbs/tools listed for the role — refusing to start if the manifest is missing, unless `ROBOFLEET_ALLOW_FULL_TOOLSET` is set (dev/test only; the all-tools fallback was the original bug that let PMs see dev verbs and 404). The optimal/docs/search/intake/secretary/git-readonly servers register their full tool surface unconditionally (role gating is server-side at the route).
...
graph LR
    subgraph AgentContainer["Agent container (per spawn)"]
        ...
        FS["robofleet-flow MCP"]
        DS["robofleet-do MCP"]
        GR["robofleet-git-readonly MCP"]
        OP["robofleet-optimal MCP"]
        DOC["robofleet-docs MCP (conditional)"]
        SRCH["robofleet-search MCP (conditional)"]
        INT["robofleet-intake MCP (grok only)"]
        SEC["robofleet-secretary MCP (grok only)"]
...
├── flow_server.py           # robofleet-flow (intent verbs)
...
│   └── _load_manifest_flow_tools / _register_tools (fails loud if no manifest; ROBOFLEET_ALLOW_FULL_TOOLSET escape hatch)
├── do_server.py             # robofleet-do (content tools)
...
│   └── _TOOLS / _load_manifest_do_tools / _register_tools (fails loud; ROBOFLEET_ALLOW_FULL_TOOLSET escape hatch)
├── optimal_server.py        # robofleet-optimal (RAG/KB)
...
├── docs_server.py           # robofleet-docs
│   ├── _handle_write/read/list/delete
│   └── create_docs_mcp_server(agent_id)
├── git_readonly.py          # robofleet-git-readonly (4 read-only tools)
├── intake_server.py         # robofleet-intake (grok only)
...
├── secretary_server.py      # robofleet-secretary (grok only; delegates to secretary_driver)
└── search_server.py         # robofleet-search (web research, Board+PM)
...
**Internal (robo-fleet):**
- `robofleet.config.settings` — `internal_api_url` (utils), `research_enabled` (orchestrator mount gate).
- `robofleet.agents_config` — `get_agent_role`, `get_agent_team` (utils headers).
- `robofleet.api.schemas.common.error_response` (utils `format_error_response`).
- `robofleet.foundation.policy.content.validators.coerce_str_list` (flow `StrList`).
- `robofleet.agent_sdk.secretary_driver` — `_do_read_state` / `_do_read_task` / `_do_submit_directive` (secretary server).
- `robofleet.mcp.schemas.WriteDocInput` (docs server).
- `robofleet.mcp.utils.ApiClient` / `format_error_response` (optimal, docs, search).
...
- `robofleet.runtime.orchestrator._generate_mcp_config` — mounts flow/do/git-readonly/optimal (always), docs (docs_roles), search (research_roles + `research_enabled`).
- `robofleet.agent_sdk.grok_intake_main` / `grok_secretary_main` — mount intake/secretary for the grok path.
- `robofleet.runtime.spawn_manifest` — writes `/app/tool-manifest.json` (the `flow_tools`/`do_tools` lists the flow/do servers read at import).

## Entry Points

- `python -m robofleet.mcp.flow_server` — `mcp.run()` at `flow_server.py:1028`. Env required: `ROBOFLEET_AGENT_ID`, `ROBOFLEET_AGENT_ROLE`; reads `ROBOFLEET_ORCHESTRATOR_URL`, `ROBOFLEET_SDK_URL`, `ROBOFLEET_TOOL_MANIFEST_PATH`.
- `python -m robofleet.mcp.do_server` — `mcp.run()` at `do_server.py:954`. Same env as flow.
- `python -m robofleet.mcp.git_readonly` — `mcp.run()` at `git_readonly.py:123`. Env: `ROBOFLEET_AGENT_ID`, `ROBOFLEET_AGENT_ROLE`, `ROBOFLEET_ORCHESTRATOR_URL`.
- `python -m robofleet.mcp.optimal_server <agent_id>` — `server.run()` at `optimal_server.py:1102`. Positional `agent_id` arg.
- `python -m robofleet.mcp.docs_server <agent_id>` — `server.run()` at `docs_server.py:251`.
- `python -m robofleet.mcp.search_server <agent_id>` — `server.run()` at `search_server.py:130`.
- `python -m robofleet.mcp.intake_server` — `mcp.run()` at `intake_server.py:215`. Env: `ROBOFLEET_API_URL`, `ROBOFLEET_PROMPTER_SESSION_ID`. Mounted by `grok_intake_main`, NOT by the orchestrator.
- `python -m robofleet.mcp.secretary_server` — `mcp.run()` at `secretary_server.py:64`. Env: `ROBOFLEET_API_URL`, `ROBOFLEET_AGENT_ID`, `ROBOFLEET_AGENT_ROLE`, `ROBOFLEET_AGENT_TOKEN`. Mounted by `grok_secretary_main`, NOT by the orchestrator.

Invocation is one subprocess per agent container (the orchestrator writes `robofleet-mcp-{agent_id}.json` into `/app/mcp-configs` and the runtime launches each `mcpServers` entry with `uv run --no-sync` pinned to `/app/.venv`).

## Config Flags

Env vars read in this slice (all `ROBOFLEET_*`):

| Flag / env | Where | Purpose |
|------------|-------|---------|
| `ROBOFLEET_AGENT_ID` | flow, do, git-readonly (required) | Agent identity for `X-Agent-ID` header + role-path. |
| `ROBOFLEET_AGENT_ROLE` | flow, do, git-readonly (required) | Role for `X-Agent-Role` + flow route prefix. |
| `ROBOFLEET_AGENT_TOKEN` | utils `_get_agent_headers` | HMAC agent token injected by orchestrator at spawn; sent as `X-Agent-Token`. |
| `ROBOFLEET_ORCHESTRATOR_URL` | flow, do, git-readonly | Orchestrator base URL (default `http://robofleet-orchestrator:8000`). |
| `ROBOFLEET_SDK_URL` | flow, do | Per-container SDK loopback for the breaker (default `http://localhost:9000`). |
| `ROBOFLEET_TOOL_MANIFEST_PATH` | flow, do | Path to the spawn manifest (default `/app/tool-manifest.json`). |
| `ROBOFLEET_API_URL` | intake, secretary | Orchestrator base URL for the grok-path servers. |
| `ROBOFLEET_PROMPTER_SESSION_ID` | intake | The live intake session id; without it `propose_draft`/`propose_batch` return a no-op string. |
| `ROBOFLEET_PROJECT_SLUG` / `ROBOFLEET_BRANCH` | set by orchestrator into `mcp_env` (consumed indirectly by `/api/git/*`) | Git context. |
| `settings.research_enabled` | orchestrator mount gate for `robofleet-search` (not read inside the slice) | Web-research server armed only when true AND role in research_roles. |
| `settings.internal_api_url` | utils `ApiClient.base_url` | Base URL for optimal/docs/search async calls. |
| `ROBOFLEET_DOCS_SITE_PROJECT_SLUG` (default `"robo-fleet-website"`) | `services/docs.py` `_refused_doc_types` | Deployer-configurable project slug named in the `write_doc(doc_type="user_facing")` refusal message — distinct from `docs_sync_*` (which stays RoboFleet-only by design for the docs-divergence sync engine, see `docs/map/engine-docs-sync.md`); defaults to RoboFleet's own value so behavior is unchanged until overridden. |
| `ROBOFLEET_DOCS_SITE_PUBLIC_URL` (default `"docs.robo-fleet.tech"`) | `services/docs.py` `_refused_doc_types` | Deployer-configurable public docs URL, same refusal message. |
| `ROBOFLEET_ALLOW_FULL_TOOLSET` | flow `_register_tools`, do `_register_tools` | Dev/test escape hatch: when set, a missing manifest registers the full tool set instead of raising `RuntimeError`. Never set in production — the full-toolset path was the original bug this policy replaced. |

No default-off feature flag is armed *inside* this slice; the only flag-gated server here is `robofleet-search` (gated upstream by `ROBOFLEET_RESEARCH_ENABLED` in the orchestrator mount).

## Gotchas

- **Import-free `__init__.py` is load-bearing.** Re-exporting server factories here would force `optimal_server` (pgvector/ollama stack, ~6s) to load on every `python -m robofleet.mcp.<name>` and time out the MCP init — symptom: "robofleet-flow/do tools never register".
- **flow/do refuse to start without the manifest** unless `ROBOFLEET_ALLOW_FULL_TOOLSET` is set. A missing `/app/tool-manifest.json` raises `RuntimeError` at import (production path). Previously the fallback registered all verbs, letting PMs call dev verbs at wrong URLs (404 storm). Local test runs without the bind mount can either set `ROBOFLEET_TOOL_MANIFEST_PATH` to a real file or set `ROBOFLEET_ALLOW_FULL_TOOLSET` to skip the hard-fail; the latter must never reach production containers.
...
- **`optimal_server` positional `agent_id` is mandatory.** `python -m robofleet.mcp.optimal_server` with no arg prints usage and exits 1. Same for docs/search.
...
CLAUDE.md "MCP servers running per agent container" table lists 5 servers (`robofleet-flow`, `robofleet-do`, `robofleet-git-readonly`, `robofleet-optimal`, `robofleet-docs`). The actual `robofleet/mcp/` directory contains **8** server modules:

- `robofleet/mcp/intake_server.py` (robofleet-intake) — omitted from the CLAUDE.md table. Mounted by `grok_intake_main` for the grok intake path, not by `_generate_mcp_config`.
- `robofleet/mcp/secretary_server.py` (robofleet-secretary) — omitted from the CLAUDE.md table. Mounted by `grok_secretary_main` for the grok secretary path.
- `robofleet/mcp/search_server.py` (robofleet-search) — omitted from the CLAUDE.md table. Mounted by `_generate_mcp_config` (orchestrator.py:2915) only when `settings.research_enabled` AND role in `(cell_pm, main_pm, product_owner, head_marketing)`.

CLAUDE.md "robofleet-optimal" row says the server exposes `robofleet_ask_mentor`, `robofleet_kb_search` only. The actual `optimal_server.py` registers **18** tools (search/rag/stats/index_code/index_docs/tokens_estimate/ask_mentor/search_error/record_error_solution/check_decision/record_decision/get_standards/validate_action/review_code/record_learning/search_learnings/clear_index/reindex_all/index_status/get_proactive_context). Understatement, not contradiction.

CLAUDE.md "robofleet-do" row lists `commit, note, say, dm, evidence` and (in the Agent Gateway section) `draft_playbook` for delivery roles + `approve_playbook`/`reject_playbook`/`archive_playbook` for the Auditor. The actual `do_server.py` `do_tools` registry also contains `pitch`, `propose_roadmap` (Product Owner only), `progress`, `notify`, `notify_list`, `notify_get`, `notify_ack`, `pr_update`, `read_messages`, `read_a2a` — none mentioned in CLAUDE.md (`do_tools` is 18 entries, not the 5 named). Understatement.
...
```
git log --oneline fd10cc862c..HEAD -- robofleet/mcp/
git diff --stat   fd10cc862c..HEAD -- robofleet/mcp/
```
...
>
> - **v0.18.0** (2026-07-04): No commit touched `robofleet/mcp/` for the X feature-spotlight workstream — `do_server.py`'s `_TOOLS` dict (21 content tools) is unchanged, which is itself the finding: `propose_feature_spotlight` was wired at the role-config + content-actions layers but never registered here. See Regression Risks below.
> - **`a0baf94b`** ("agnosticism-residue", audit item B8): `services/docs.py`'s `write_doc(doc_type="user_facing")` refusal message stops hardcoding "robo-fleet-website"/"docs.robo-fleet.tech" — the module-level `REFUSED_DOC_TYPES` constant becomes `_refused_doc_types()`, a function reading the new `ROBOFLEET_DOCS_SITE_PROJECT_SLUG`/`ROBOFLEET_DOCS_SITE_PUBLIC_URL` settings live on every call (both default to RoboFleet's own values, so behavior is unchanged for this deployment). This `services/docs.py` file was previously undocumented in this slice's Files table — added above alongside the fix since it's the `docs_server.py` MCP server's sole backing service.
