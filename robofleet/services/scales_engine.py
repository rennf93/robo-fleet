...
Mirrors ``robofleet.services.periscope_engine.PeriscopeEngine``'s org-scope shape
...
against the RoboFleet project purely as an anchor) — and ``PestControlEngine``'s
...
from robofleet.services.task import SCALES_SOURCE, TaskCreateRequest, get_task_service
...
class ScalesEngine(BaseService):
    ...
    async def run_cycle(self) -> TaskTable | None:
        ...
        if project is None or project.id is None:
            self.log.warning(
                "scales-engine: RoboFleet project not resolvable; skipping"
            )
            return None
