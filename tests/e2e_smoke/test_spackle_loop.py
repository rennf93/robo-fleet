"""Scenario: the Spackle (Board Program) loop end to end.

Mirrors test_pest_control_loop.py: arming spackle via the settings-store key
...
item through the real ``SpackleService`` moves the LEARN ledger's counters.
"""
...
def _arm_and_opt_in(stack: E2EStack, project_id: Any) -> None:
    ...
    async def _run(session: AsyncSession) -> None:
        session.add(
            SystemSettingTable(key="board_program.spackle.enabled", value="true")
        )
        project = (
        ...
        project.board_programs = ["spackle"]
...
def _find_spackle_task(stack: E2EStack) -> dict[str, Any]:
    ...
    async def _run(session: AsyncSession) -> dict[str, Any]:
        rows = (
            (
                await session.execute(
                    select(TaskTable).where(TaskTable.source == SPACKLE_SOURCE)
                )
            )
            .scalars()
            .all()
        ...
        row = (
            (
                await session.execute(
                    ...
                    .where(BoardProgramCycleTable.program_key == "spackle")
...
def _approve_fake_item(
    ...
    async def _run(session: AsyncSession) -> str:
        ...
        await session.flush()
        result = await get_spackle_service(session).approve_item(
...
def test_spackle_loop_originates_dedups_and_records(
    e2e_stack: E2EStack,
) -> None:
    stack = e2e_stack
    ...
    assert opened == ["spackle"], opened

    state = _find_spackle_task(stack)
    ...
    state_after = _find_spackle_task(stack)
    assert state_after["count"] == ONE, state_after

    # Approve a fake item on the open cycle through the real SpackleService —
