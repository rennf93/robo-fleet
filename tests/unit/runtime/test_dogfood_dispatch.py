"""Dogfood exploration dispatch — Product-Owner-solo, never the
...
def _dogfood_task(
    *,
    ...
    return {
        ...
        "team": "board",
        "title": "Dogfood walk",
        "description": "Walk the product as a user and propose a friction audit.",
...
@pytest.mark.asyncio
async def test_dogfood_dispatch_spawns_only_product_owner() -> None:
    """A dogfood exploration task must spawn the Product Owner alone."""
    orch = _make_orch()
    task = _dogfood_task()
    with (
...
@pytest.mark.asyncio
async def test_dogfood_dispatch_skips_once_authored() -> None:
    """Once ``propose_friction_fixes`` has stamped the friction_fixes marker,
    the dispatcher must not spawn again."""
    orch = _make_orch()
    task = _dogfood_task(orchestration_markers={"friction_fixes": {"items": []}})
    with (
        patch.object(orch, "_is_agent_active", return_value=False),
        patch.object(orch, "spawn_agent", new=AsyncMock()) as spawn,
    ):
...
@pytest.mark.asyncio
async def test_dogfood_dispatch_skips_active_po() -> None:
    orch = _make_orch()
    task = _dogfood_task()
    with (
...
@pytest.mark.asyncio
async def test_dispatch_pm_work_routes_dogfood_away_from_board_handler() -> None:
    """A board_dogfood task must ride the dedicated dispatcher, never the
    ...
    stub._dispatch_dogfood_exploration = AsyncMock()
    ...
    stub._dispatch_dogfood_exploration.assert_awaited_once()
    stub._handle_board_assigned_task.assert_not_awaited()
    stub._handle_pm_assigned_task.assert_not_awaited()


@pytest.mark.asyncio
async def test_dogfood_tasks_are_never_routed_by_dev_dispatch() -> None:
    tasks = [_dogfood_task()]
    stub = MagicMock()
...
def test_dogfood_prompt_names_solo_po_and_real_verbs() -> None:
    orch = _make_orch()
    prompt = orch._build_dogfood_prompt(_dogfood_task())
    assert "triage()" in prompt
    assert "propose_friction_fixes(" in prompt
    assert "i_am_idle()" in prompt
    assert "do not" in prompt.lower()


def test_dogfood_prompt_omits_optional_sections_when_empty() -> None:
    orch = _make_orch()
    prompt = orch._build_dogfood_prompt(_dogfood_task())
    assert "## Prior cycles" not in prompt


def test_dogfood_prompt_renders_prior_cycles_when_given() -> None:
    orch = _make_orch()
    prompt = orch._build_dogfood_prompt(
        _dogfood_task(),
...
def test_dogfood_prompt_names_browser_tools() -> None:
    """The ONE board-program prompt referencing playwright tools — proves
    the prompt text matches the task-scoped MCP grant (see
    test_mcp_playwright_gating.py's dogfood-specific tests)."""
    orch = _make_orch()
    prompt = orch._build_dogfood_prompt(_dogfood_task())
...
def test_dogfood_prompt_omits_panel_url_for_other_project(
    ...
    assert "https://panel.example.com" not in prompt


@pytest.mark.asyncio
async def test_dogfood_dispatch_injects_prior_context() -> None:
    orch = _make_orch()
    task = _dogfood_task()
    with (
        patch.object(orch, "_is_agent_active", return_value=False),
