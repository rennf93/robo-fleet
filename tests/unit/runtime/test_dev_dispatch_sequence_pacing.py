...
async def test_ready_dev_task_still_spawns() -> None:
    ...
    with (
        ...
        patch.object(
            orch, "_blocked_by_earlier_lane_sibling", new=AsyncMock(return_value=False)
        ),
        patch.object(
            orch, "_pm_respawn_should_gate", new=AsyncMock(return_value=False)
        ),
        patch.object(
            orch, "_validate_task_for_spawn", new=AsyncMock(return_value=None)
        ),
        patch.object(
            orch, "_get_prompt_for_agent", new=AsyncMock(return_value="prompt")
        ),
