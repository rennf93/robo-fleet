"""Scenario: the Librarian (Board Program) loop end to end.

Mirrors test_sentinel_loop.py's arm -> originate -> dedup shape (same
...
Unlike a quality report, propose_playbook_drafts' output is REAL playbook
rows (not just a task marker) — this asserts they land as PENDING/DRAFT
playbooks riding the normal curation queue, in addition to the mining task
completing.
"""
...
def _seed_system_and_auditor(stack: E2EStack) -> str:
    ...
    project those two own — ``LibrarianEngine._originate`` writes
    ``created_by``/``assigned_to`` straight from the static identity
    registry (not ``seed_company``'s random ``uuid4()`` agents; mirrors
    test_sentinel_loop.py's identical ``_seed_system_and_auditor``), and the
...
def _arm(stack: E2EStack, project_slug: str) -> None:
    ...
    seeded project (the RoboFleet-project resolution roadmap/x_feature/
        ...
        rows = (
            (
                await session.execute(
                    select(TaskTable).where(TaskTable.source == LIBRARIAN_SOURCE)
                )
            )
            .scalars()
            .all()
...
def _drafts_marker(stack: E2EStack, task_id: Any) -> dict[str, Any] | None:
    ...
    async def _run(session: AsyncSession) -> dict[str, Any] | None:
        ...
        return markers.get_playbook_drafts(row)
...
def _playbooks(stack: E2EStack) -> list[dict[str, Any]]:
    ...
    async def _run(session: AsyncSession) -> list[dict[str, Any]]:
        rows = (await session.execute(select(PlaybookTable))).scalars().all()
        return [
            {
                "id": r.id,
                "title": r.title,
...
def test_librarian_loop_originates_dedups_and_completes(
    e2e_stack: E2EStack,
) -> None:
    stack = e2e_stack
    ...
    assert opened == ["librarian"], opened

    state = _find_librarian_task(stack)
    ...
    state_after = _find_librarian_task(stack)
    assert state_after["count"] == ONE, state_after

    cycle_before = _cycle_state(stack)
    assert cycle_before["open_cycle"] is True

    # The Auditor authors the drafts through the REAL do_server module +
    # /api/v1/do/propose_playbook_drafts route + ContentActions + LIBRARIAN
    # service wiring — the same wiring-regression class check
    # test_sentinel_loop.py guards (a verb granted in role_config but missing
    # from do_server._TOOLS is unreachable over MCP).
    ...
    assert "propose_playbook_drafts" in do_module._REGISTERED_TOOLS, (
        "propose_playbook_drafts is granted to auditor in role_config but "
        "absent from this agent's _register_tools() output — the manifest "
        "-> _register_tools -> callable chain dropped it"
    )
    # And the invariant this whole program preserves: the Auditor's manifest
    # must NOT also carry draft_playbook (Coroner's precedent, unchanged).
    assert "draft_playbook" not in do_module._REGISTERED_TOOLS

    drafts = [
        {
            "title": "Verify venv freshness before make quality",
            "body": "Check the venv's dev-extra marker before running the gate "
            "so a stale toolchain never silently skips the DB-backed suite.",
            "pattern_evidence": "This exact failure recurred across at least two "
            "independent tasks' journals this cycle.",
        },
        {
            "title": "Rebase onto live slave before opening a stacked PR",
            "body": "Re-fetch the base branch immediately before cutting a new "
            "branch so a squash-merged parent's history is never silently stale.",
            "pattern_evidence": "Two separate journal entries this cycle name the "
            "same stale-base mistake.",
        },
    ]
    env = expect_ok(
        auditor.do("propose_playbook_drafts", drafts=drafts),
        "auditor propose_playbook_drafts",
    )
    assert env.get("status") == "playbook_drafts_proposed", env
    task_id_str = env.get("task_id")
    assert task_id_str == str(row["id"]), (
        "unlike x_feature's separate materialized draft, propose_playbook_"
        f"drafts completes the SAME exploration task: {env}"
    )
    ...
    payload = _drafts_marker(stack, task_id)
    assert payload is not None
    assert len(payload["drafts"]) == TWO
    assert payload["drafts"][0]["title"] == drafts[0]["title"]

    # The drafts are REAL playbook rows, PENDING curation (status=draft) —
    # not just a task marker, unlike a quality report / market brief.
    playbooks = _playbooks(stack)
    assert len(playbooks) == TWO
    titles = {p["title"] for p in playbooks}
    assert titles == {drafts[0]["title"], drafts[1]["title"]}
    for p in playbooks:
        assert p["status"] == "draft"
        assert "librarian" in p["tags"]

    # The exploration going terminal auto-closes the LEARN ledger row — no
    # manual close call is needed; propose_playbook_drafts only sets the
    # task status, and BoardProgramEngine's own dedup check reconciles the
    # rest.
