...
from robofleet.api.utils.optimal import kb_denial_response as _kb_denial_response
...
from robofleet.services.optimal_brain import (
    get_mentor_service,
...
async def search(
    ...
    try:
        async with asyncio.timeout(search_timeout):
            service = await get_optimal_service()
            # Use search_with_gaps when available to surface per-index timeout
            # gaps; fall back to search() + empty gaps for backwards compat.
...
async def record_learning(
    ...
    # moves on instead of looping on a 500.
    try:
