...
You do **not** call any tool to create a PR. There is no `robofleet_git_create_pr` MCP tool.
...
There is no `robofleet_git_merge_pr` MCP tool.
...
- `NO_PR` on `pass`/`fail` → the choreographer didn't open a PR; check the workspace state with `robofleet_git_status` and re-call `open_pr` once the workspace is clean.
