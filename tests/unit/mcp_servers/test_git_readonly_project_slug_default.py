...
robofleet-git-readonly tool — agents no longer need to guess a slug, and the
RAG-taught literal "robo-fleet" example (a non-existent slug) can't 404 them.
"""
...
def test_resolve_omitted_falls_back_to_env(
    ...
    monkeypatch.setenv("ROBOFLEET_PROJECT_SLUG", "robofleet-api")
    assert git_module._resolve_project_slug(None) == "robofleet-api"
    assert git_module._resolve_project_slug("") == "robofleet-api"
...
def test_git_status_omitted_slug_uses_env(
    ...
    with patch("httpx.Client") as client_cls:
        ...
        git_module.robofleet_git_status()
        _, kwargs = client.get.call_args
        assert kwargs["params"]["project_slug"] == "robofleet-api"
...
def test_git_status_missing_both_returns_error_without_http_call(
    ...
    with patch("httpx.Client") as client_cls:
        result = git_module.robofleet_git_status()
