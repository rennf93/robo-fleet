"""Pest Control (Board Program) engine API — the CEO approves/rejects items
within a held bug-hunt cycle. CEO-only throughout. Approving an item
materializes it as a BACKLOG task; nothing here starts it — normal PM
activation takes it from there. Mirrors ``robofleet.api.routes.roadmap``.
"""

from uuid import UUID

from fastapi import APIRouter, HTTPException, status

from robofleet.api.deps import CurrentAgentContext, DbSession
from robofleet.api.schemas.pest_control import (
    PestHuntCycleResponse,
    PestHuntItemActionResponse,
    PestHuntRejectRequest,
)
from robofleet.api.utils.pest_control import require_ceo as _require_ceo
from robofleet.api.utils.pest_control import to_response as _to_response
from robofleet.foundation.policy.content import markers
from robofleet.security import guard_deco
from robofleet.services.pest_control_service import get_pest_control_service

router = APIRouter()


@router.get("/cycles", response_model=list[PestHuntCycleResponse])
async def list_pest_control_cycles(
    db: DbSession, agent: CurrentAgentContext
) -> list[PestHuntCycleResponse]:
    """Every open pest-control cycle already authored by the Product Owner.

    A cycle the PO hasn't authored yet (no items drafted) is omitted — there
    is nothing for the CEO to review until ``propose_bug_hunt`` lands.
    """
    _require_ceo(agent)
    tasks = await get_pest_control_service(db).list_open_cycles()
    return [_to_response(t) for t in tasks if markers.get_pest_hunt(t)]


@router.post(
    "/cycles/{task_id}/items/{item_id}/approve",
    response_model=PestHuntItemActionResponse,
)
@guard_deco.rate_limit(requests=30, window=60)
@guard_deco.block_clouds()
async def approve_pest_hunt_item(
    task_id: UUID,
    item_id: str,
    db: DbSession,
    agent: CurrentAgentContext,
) -> PestHuntItemActionResponse:
    """Materialize one proposed item as a BACKLOG task (idempotent)."""
    _require_ceo(agent)
    result = await get_pest_control_service(db).approve_item(
        task_id, item_id, created_by=agent.agent_id
    )
    if result is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No such open pest-control item",
        )
    await db.commit()
    return PestHuntItemActionResponse(
        status=result.status,
        item_id=result.item_id,
        materialized_task_id=result.materialized_task_id,
        detail=result.detail,
    )


@router.post(
    "/cycles/{task_id}/items/{item_id}/reject",
    response_model=PestHuntItemActionResponse,
)
@guard_deco.rate_limit(requests=30, window=60)
@guard_deco.block_clouds()
@guard_deco.content_type_filter(["application/json"])
@guard_deco.honeypot_detection(["email", "phone", "website"])
async def reject_pest_hunt_item(
    task_id: UUID,
    item_id: str,
    data: PestHuntRejectRequest,
    db: DbSession,
    agent: CurrentAgentContext,
) -> PestHuntItemActionResponse:
    """Reject one proposed item with a reason (idempotent)."""
    _require_ceo(agent)
    result = await get_pest_control_service(db).reject_item(
        task_id, item_id, data.reason
    )
    if result is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No such open pest-control item",
        )
    await db.commit()
    return PestHuntItemActionResponse(
        status=result.status,
        item_id=result.item_id,
        materialized_task_id=result.materialized_task_id,
        detail=result.detail,
    )
