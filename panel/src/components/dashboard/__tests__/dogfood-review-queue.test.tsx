          ...
          items: [
            {
              ...
              priority: 2,
              evidence:
                "Walked /settings -> changed panel_base_url -> clicked Save -> no toast, no spinner, unclear if it saved",
              status: "proposed",
            },
            {
              id: "item-1",
...
vi.mock("@/lib/api", () => ({
  dogfoodApi: { listCycles, approveItem, rejectItem },
}));
...
describe("DogfoodReviewQueue", () => {
  ...
  it("renders both item drafts with their walked-path evidence", async () => {
    render(withQueryClient(<DogfoodReviewQueue />));
    expect(
      await screen.findByText("Settings save button gives no feedback"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Docs site 404 on /docs/board-programs"),
    ).toBeInTheDocument();
    expect(screen.getByText(/Walked \/settings/)).toBeInTheDocument();
  });
  ...
  it("renders nothing when there is no authored cycle", async () => {
    ...
    const { container } = render(withQueryClient(<DogfoodReviewQueue />));
