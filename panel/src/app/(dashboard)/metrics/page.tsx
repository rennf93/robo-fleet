...
function TeamHealthCard({
  ...
  return (
    <Card>
      ...
      <CardContent>
        <HelpTip label="100 minus 20 points per blocked task — a rough at-a-glance score, not a precise metric">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <HelpTip label="Time window — narrows every card, chart, and table below to this period">
          <div>
            <SegmentedControl
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <HelpTip label="Share of tokens served from the prompt cache instead of a fresh model call">
            <div>
...
function SpawnWasteCard({ data, isLoading }: SpawnWasteCardProps) {
  return (
    <Card>
      <CardHeader className="pb-2">
        <HelpTip label="An agent spawn that made no commit and left no trace of progress before exiting">
          <CardTitle className="text-base flex items-center gap-2">
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            ...
            {data.respawn_strikes.length > 0 && (
              <HelpTip label="Tasks the respawn-loop breaker has flagged for repeatedly re-spawning the same agent with no progress">
                <p className="text-xs text-muted-foreground">
                  {data.respawn_strikes.length} wedged task
                  {data.respawn_strikes.length === 1 ? "" : "s"} with open
                  respawn strikes
                </p>
              </HelpTip>
            )}
