...
from robofleet.services.governance import GovernanceService
...
async def _add_convention_finding(
    ...
    *,
    ...
    finding = ProjectConventionFindingTable(
        ...
        task_id=task_id,
        file="robofleet/services/foo.py",
        line=42,
...
async def test_conventions_verdict_counts_block_and_warn(
    ...
    # Need a project for the convention finding FK. Keep the id in a typed
    # local so the helper gets a uuid.UUID, not the Mapped[UUID] column value
    # (mypy infers the SQLAlchemy UUID type off the ORM attribute).
    project_id = uuid4()
    project = ProjectTable(
        id=project_id,
        name="gov test project",
        slug=f"gov-test-{project_id.hex[:8]}",
