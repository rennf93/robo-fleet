...
Board Program registry entry (``robofleet.foundation.policy.board_programs``)
...
async def _originate_roadmap(session: AsyncSession) -> TaskTable | None:
    from robofleet.services.roadmap_engine import get_roadmap_engine
...
async def _originate_spackle(session: AsyncSession) -> TaskTable | None:
    from robofleet.services.spackle_engine import get_spackle_engine

    return await get_spackle_engine(session).run_cycle()


async def _originate_mirror(session: AsyncSession) -> TaskTable | None:
    from robofleet.services.mirror_engine import get_mirror_engine
...
async def _originate_war_room(session: AsyncSession) -> TaskTable | None:
    ...
    from robofleet.services.war_room_engine import get_war_room_engine

    return await get_war_room_engine(session).run_cycle()


async def _originate_barfly(session: AsyncSession) -> TaskTable | None:
    from robofleet.services.barfly_engine import get_barfly_engine
...
async def _originate_dogfood(session: AsyncSession) -> TaskTable | None:
    ...
    ``robofleet.services.dogfood_engine``'s module docstring."""
    from robofleet.services.dogfood_engine import get_dogfood_engine
...
async def _pest_control_rework_spike(session: AsyncSession) -> bool:
    ...
    from robofleet.services.metrics import get_metrics_service
