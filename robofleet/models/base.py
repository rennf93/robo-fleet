...
# AgentRole and Team are canonicalized in robofleet/foundation/identity.py.
# These bindings keep existing imports (`from robofleet.models.base import
...
class ModelProvider(StrEnum):
    ...
    provider (robofleet.llm.providers.grok), not ANTHROPIC_BASE_URL injection.
    The xAI key is set via PUT /api/providers/grok/key.
    `OPENAI` routes through the official Codex CLI on a ChatGPT subscription
    (robofleet.llm.providers.codex.CodexCliProvider), mirroring GROK's shape: a
    ...
    dedicated provider (robofleet.llm.providers.gemini), never
    ...
    dedicated provider (robofleet.llm.providers.kimi.KimiCliProvider), never
