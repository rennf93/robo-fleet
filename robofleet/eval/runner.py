...
Replays ``robofleet/eval/fixtures.py``'s ``BenchTaskSpec`` fixtures through the
...
polling/scoring/DB plumbing without touching Docker. ``python -m robofleet.eval
...
def _seed_company(stack: E2EStack, slugs: Iterable[str]) -> None:
    ...
    from robofleet.models import AgentStatus
...
def _seed_bench_cell(
    ...
    only the CEO may merge (``robofleet.services.git``'s ``CEO_ONLY`` check) —
    ...
    from robofleet.services.task import EVAL_BENCH_SOURCE, get_task_service
...
def _bench_environment(dev_slug: str) -> Iterator[BenchEnvironment]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "in containers or wheels — run `python -m robofleet.eval` from a git "
    ...
    with _scratch_database() as db_url:
        ...
        try:
            ...
            with stack_cm(db_url, _ScratchTmpFactory(root_path)) as stack:
                ...
                # `grep obsidian_vault_enabled robofleet/services/{task,journal,
...
class OrchestratorStageSpawner:
    ...
    async def run_stage(self, *, task: dict[str, Any], agent_slug: str) -> None:
        from robofleet.models.runtime import OrchestratorAgentState
...
async def _drive_task_to_terminal(
    ...
    *,
    ...
    """
    from robofleet.runtime.orchestrator import _system_api_headers
