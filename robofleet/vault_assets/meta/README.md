...
This vault is a projection of RoboFleet's live task/journal/A2A state — read-only from the org's point of view (edits here never flow back into RoboFleet; see `docs.robo-fleet.tech` for the full model).
...
Regenerate at any time with `python -m robofleet.vault rebuild` (safe — a task's Auditor-authored narrative is preserved across a rebuild).
