...
class _FakeTask:
    ...
    ``markers`` and ``propose_gap_fill`` touch."""
...
def _default_project_lookup(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    propose_gap_fill call. Every test predating that check builds its
...
@pytest.mark.asyncio
async def test_propose_gap_fill_forbidden_for_non_po() -> None:
    env = await _actions("head_marketing").propose_gap_fill(
...
@pytest.mark.asyncio
async def test_propose_gap_fill_forbidden_for_developer() -> None:
    env = await _actions("developer").propose_gap_fill(
        agent_id=uuid4(), items=_valid_items(2)
    )
    assert env.error == "not_authorized"
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_empty_items() -> None:
    env = await _actions("product_owner").propose_gap_fill(agent_id=uuid4(), items=[])
    assert env.error == "invalid_state"


@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_too_many_items() -> None:
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_missing_evidence() -> None:
    bad = _valid_item(0)
    del bad["evidence"]
    env = await _actions("product_owner").propose_gap_fill(
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_oversized_evidence() -> None:
    bad = _valid_item(0)
    bad["evidence"] = "x" * 2001
    env = await _actions("product_owner").propose_gap_fill(
        agent_id=uuid4(), items=[bad]
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_too_short_evidence() -> None:
    """Below the 20-char anti-soup floor (``_GAP_FILL_ITEM_TEXT_FIELDS``) —
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_placeholder_soup_evidence() -> None:
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_missing_field() -> None:
    bad = _valid_item(0)
    del bad["description"]
    env = await _actions("product_owner").propose_gap_fill(
        agent_id=uuid4(), items=[bad]
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_unknown_team() -> None:
    bad = _valid_item(0)
...
@pytest.mark.asyncio
async def test_propose_gap_fill_no_open_cycle_is_invalid_state(
...
@pytest.mark.asyncio
async def test_propose_gap_fill_persists_audit_onto_open_task(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    ...
    env = await actions.propose_gap_fill(agent_id=agent_id, items=items)
    assert env.error is None
    assert env.status == "gap_fill_proposed"
    assert env.task_id == str(cycle_task.id)

    payload = markers.get_gap_fill(cycle_task)
...
@pytest.mark.asyncio
async def test_propose_gap_fill_sends_telegram_push_per_item(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    for i, call in enumerate(notify.notify_ceo_of_queue_item.await_args_list):
        assert call.kwargs["kind"] == "spackle"
...
@pytest.mark.asyncio
async def test_propose_gap_fill_survives_telegram_push_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    env = await actions.propose_gap_fill(agent_id=agent_id, items=_valid_items(1))
...
@pytest.mark.asyncio
async def test_propose_gap_fill_ignores_cycle_assigned_to_another_agent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    other_agent = uuid4()
...
@pytest.mark.asyncio
async def test_propose_gap_fill_rejects_item_targeting_unopted_project(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The positive-gate mirror of propose_roadmap's exclusion check: an item
    targeting a project that has NOT opted into spackle is refused at
...
@pytest.mark.asyncio
async def test_propose_gap_fill_allows_opted_project_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    env = await actions.propose_gap_fill(agent_id=agent_id, items=[bad])
    assert env.error is None


@pytest.mark.asyncio
async def test_propose_gap_fill_allows_unresolvable_project_slug_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_propose_gap_fill_ignores_already_authored_cycle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    authored_task = _FakeTask(
        assigned_to=agent_id,
        orchestration_markers={"gap_fill": {"items": []}},
    )
    task_svc = MagicMock()
