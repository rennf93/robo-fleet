"""MirrorService — the CEO's per-item approve/reject glue over a held
mirror (Board Program) cycle.

The mirror engine opens a HELD exploration task (``board_mirror`` source);
the Head of Marketing authors the messaging audit onto it via
``propose_messaging_fixes`` (1-5 evidence-backed item drafts, persisted as a
marker payload — see ``robofleet.foundation.policy.content.markers.
...
from robofleet.foundation.policy.board_programs import PROGRAMS, project_participates
from robofleet.foundation.policy.content import markers
from robofleet.models.base import TaskStatus, Team
from robofleet.services.base import BaseService
from robofleet.services.board_programs import learn_ref
from robofleet.services.task import MIRROR_ITEM_SOURCE, MIRROR_SOURCE
...
class MessagingFixItemResult:
    """Outcome of an approve/reject call on one messaging-fix item.
...
class MirrorService(BaseService):
    """List / approve / reject items within the open mirror cycle(s)."""

    service_name = "mirror_service"

    async def list_open_cycles(self) -> list[TaskTable]:
        """Every open (non-terminal) mirror exploration task, authored or
        not."""
        from robofleet.services.task import get_task_service

        return await get_task_service(self.session).list_open_mirror_cycles()

    async def approve_item(
        self, task_id: UUID, item_id: str, *, created_by: UUID
    ) -> MessagingFixItemResult | None:
        """Materialize one proposed item as a BACKLOG docs task.

        Returns None when ``task_id`` is not an open mirror cycle or
    ...
    async def _find_item(
        ...
        """Resolve (exploration task, messaging-fixes payload, one item) or
        (None, None, None). Deep-copies the stored marker before mutating it
        — see ``SpackleService._find_item``'s identical dirty-check
        rationale."""
        from robofleet.services.task import get_task_service

        task = await get_task_service(self.session).get(task_id)
        if task is None or task.source != MIRROR_SOURCE:
            return None, None, None
        stored = markers.get_messaging_fixes(task)
        if stored is None:
            return None, None, None
    ...
    async def _materialize(
        ...
        if not project_participates(PROGRAMS["mirror"], project.board_programs):
            self.log.warning(
                "mirror: materialize skipped — project not opted in",
                project_slug=item["project_slug"],
            )
            raise ValueError(
                f"project {item['project_slug']!r} is not opted into the mirror program"
            )
        draft = {
            "title": item["title"],
    ...
    async def _record_learn(
        ...
        try:
            ...
            self.log.warning("mirror: LEARN record_decision failed (best-effort)")


def get_mirror_service(session: AsyncSession) -> MirrorService:
    """Construct a MirrorService bound to ``session``."""
    return MirrorService(session)
