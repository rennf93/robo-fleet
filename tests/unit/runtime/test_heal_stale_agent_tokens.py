...
The heal scans ``AGENT_IMAGES`` for local persistent-container slugs. Leg D1
...
containers (intake/secretary). These tests target the ``intake-1`` slug, the
surviving ``AGENT_IMAGES`` entry, instead of the removed ``be-dev-1``.
"""
...
# intake-1 is a surviving AGENT_IMAGES entry (interactive persistent container).
_INTAKE_SLUG = "intake-1"
_INTAKE_UUID = AGENT_UUIDS[_INTAKE_SLUG]
_INTAKE_ROLE = "prompter"
_INTAKE_TEAM = "board"
_INTAKE_CONTAINER = f"robofleet-agent-{_INTAKE_SLUG}"
...
async def test_heal_kills_stale_token_containers(
    ...
    orch = _orch()
    # intake-1 holds a token signed with a DIFFERENT secret (rotated after spawn).
    stale_token = issue_agent_token(_INTAKE_UUID, _INTAKE_ROLE, _INTAKE_TEAM)
    monkeypatch.setenv("ROBOFLEET_AGENT_AUTH_SECRET", secrets.token_hex(32))

    async def inspect(name: str) -> tuple[bool, int | None]:
        return (name == _INTAKE_CONTAINER, 0)

    orch._inspect_container_state = AsyncMock(side_effect=inspect)
    orch._read_container_auth_env = AsyncMock(
        return_value=(stale_token, _INTAKE_UUID, _INTAKE_ROLE)
    )
    removed: list[str] = []
    ...
    assert n == 1
    assert removed == [_INTAKE_CONTAINER]
...
async def test_heal_leaves_valid_token_containers(
    ...
    monkeypatch.setenv("ROBOFLEET_AGENT_AUTH_SECRET", secrets.token_hex(32))
    orch = _orch()
    valid_token = issue_agent_token(_INTAKE_UUID, _INTAKE_ROLE, _INTAKE_TEAM)
    ...
    orch._inspect_container_state = AsyncMock(side_effect=inspect)
    orch._read_container_auth_env = AsyncMock(
        return_value=(valid_token, _INTAKE_UUID, _INTAKE_ROLE)
    )
    orch._remove_container = AsyncMock()
...
async def test_heal_inert_when_secret_unset(
    ...
    orch._read_container_auth_env = AsyncMock(
        return_value=("UNSIGNED", _INTAKE_SLUG, _INTAKE_ROLE)
    )
    orch._remove_container = AsyncMock()
...
async def test_heal_kills_slug_env_container_with_slug_signed_token(
    ...
    slug_signed_token = issue_agent_token(_INTAKE_SLUG, _INTAKE_ROLE, _INTAKE_TEAM)
    ...
    orch._inspect_container_state = AsyncMock(side_effect=inspect)
    orch._read_container_auth_env = AsyncMock(
        return_value=(slug_signed_token, _INTAKE_SLUG, _INTAKE_ROLE)
    )
    removed: list[str] = []
    orch._remove_container = AsyncMock(
        side_effect=lambda name, **_: removed.append(name)
...
def test_heal_accepts_expiring_format_token(
    ...
    uuid = AGENT_UUIDS.get(_INTAKE_SLUG, _INTAKE_SLUG)
    tok = issue_agent_token(uuid, _INTAKE_ROLE, _INTAKE_TEAM, ttl_seconds=3600)
    assert verify_agent_token(tok, uuid, _INTAKE_ROLE, _INTAKE_TEAM) is True
