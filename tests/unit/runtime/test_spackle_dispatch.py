"""Spackle exploration dispatch — Product-Owner-solo, never the
two-reviewer board-review gate, never the dev/PM delivery dispatchers.
Mirrors test_pest_control_dispatch.py.
"""
...
def _spackle_task(
    *, orchestration_markers: dict[str, Any] | None = None
    ...
    return {
        ...
        "description": "Audit half-shipped surface area and propose a gap-fill audit.",
...
@pytest.mark.asyncio
async def test_spackle_dispatch_spawns_only_product_owner() -> None:
    """A spackle exploration task must spawn the Product Owner alone."""
    orch = _make_orch()
    task = _spackle_task()
    with (
...
@pytest.mark.asyncio
async def test_spackle_dispatch_skips_once_authored() -> None:
    """Once ``propose_gap_fill`` has stamped the gap_fill marker, the
    dispatcher must not spawn again."""
    orch = _make_orch()
    task = _spackle_task(orchestration_markers={"gap_fill": {"items": []}})
    with (
        patch.object(orch, "_is_agent_active", return_value=False),
        patch.object(orch, "spawn_agent", new=AsyncMock()) as spawn,
    ):
...
@pytest.mark.asyncio
async def test_spackle_dispatch_skips_active_po() -> None:
    orch = _make_orch()
    task = _spackle_task()
    with (
...
@pytest.mark.asyncio
async def test_dispatch_pm_work_routes_spackle_away_from_board_handler() -> None:
    """A board_spackle task must ride the dedicated dispatcher, never the
    ...
    stub._dispatch_spackle_exploration.assert_awaited_once()
    stub._handle_board_assigned_task.assert_not_awaited()
    stub._handle_pm_assigned_task.assert_not_awaited()


@pytest.mark.asyncio
async def test_spackle_tasks_are_never_routed_by_dev_dispatch() -> None:
    tasks = [_spackle_task()]
    stub = MagicMock()
...
def test_spackle_prompt_names_solo_po_and_real_verbs() -> None:
    orch = _make_orch()
    prompt = orch._build_spackle_prompt(_spackle_task())
    assert "triage()" in prompt
    assert "propose_gap_fill(" in prompt
    assert "i_am_idle()" in prompt
    assert "do not" in prompt.lower()


def test_spackle_prompt_omits_optional_sections_when_empty() -> None:
    orch = _make_orch()
    prompt = orch._build_spackle_prompt(_spackle_task())
    assert "## Prior cycles" not in prompt


def test_spackle_prompt_renders_prior_cycles_when_given() -> None:
    orch = _make_orch()
...
@pytest.mark.asyncio
async def test_spackle_dispatch_injects_prior_context() -> None:
    orch = _make_orch()
    task = _spackle_task()
    with (
        patch.object(orch, "_is_agent_active", return_value=False),
