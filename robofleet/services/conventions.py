...
committed ``.robofleet/conventions.yml``), caches it per ``(project, HEAD sha)``,
...
from robofleet.services.git import CONVENTIONS_SCAFFOLD_BRANCH, get_git_service
...
class ConventionsService(BaseService):
    ...
    async def render_ambient_block(
        ...
        if len(kept) < len(constrained):
            ...
            kept.append(
                f"- (+{extra} more module{plural} — see .robofleet/conventions.yml)"
            )
    ...
    async def scaffold(
        ...
        """Open a PR adding the auto-scaffolded ``.robofleet/conventions.yml``."""
        mapping = await self.get_map(project, workspace=workspace)
    ...
    def _read_committed_standard(
        ...
        if root is None:
            return None, "missing"
        path = root / ".robofleet" / "conventions.yml"
        if not path.is_file():
            return None, "missing"
        try:
            text = path.read_text()
    ...
    async def _publish(
        ...
        *,
        ...
        title = f"chore(conventions): {action} .robofleet/conventions.yml"
        body = (
            "Auto-generated by RoboFleet's architectural-conventions standard. "
