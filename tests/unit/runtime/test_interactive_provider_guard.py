"""Codex (OPENAI) and Kimi (KIMI) are V1 delivery-roles-only - neither has an
interactive-session driver image (unlike GROK's dedicated GROK_PROMPTER_IMAGE /
GROK_SECRETARY_IMAGE and the Gemini/ADK GEMINI_PROMPTER_IMAGE /
GEMINI_SECRETARY_IMAGE). Routing a delivery-only provider to the persistent
Intake/Secretary agent must refuse loudly instead of silently falling through
to a mismatched provider env.
"""
...
class TestRejectInteractiveUnsupportedProvider:
    ...
    @pytest.mark.parametrize("provider", [ModelProvider.OPENAI, ModelProvider.KIMI])
