...
* **json mode (fallback — not emitted by our entrypoint).** ``--output-format
  ...
  ``robofleet.billing.pricing``, so it prices at the full input rate rather than
...
``robofleet.billing.pricing.calculate_cost`` and the per-model costs are summed
...
# rate-limit detector; see robofleet.runtime.orchestrator._GEMINI_RATE_LIMIT_EXIT_CODE).
