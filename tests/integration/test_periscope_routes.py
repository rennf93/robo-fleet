"""Periscope (Board Program) engine route coverage — CEO-only, read-only.
Mirrors test_pest_control_routes.py minus the approve/reject routes: a
market brief is a report, not a queue item."""
...
async def _seed_completed_brief(session: AsyncSession) -> TaskTable:
    ...
    markers.set_market_brief(
        task,
        {
            ...
            "positioning_note": "Emphasize the findings ledger",
            "injection_hits": [],
        },
    )
    await session.flush()
...
def _build_app(db_session: AsyncSession, role: AgentRole, agent_id: UUID) -> FastAPI:
    app = FastAPI()
    app.include_router(periscope_router, prefix="/api/periscope")
...
async def ceo_client(db_session: AsyncSession) -> AsyncIterator[AsyncClient]:
    ...
    # real-DB suite could still leave a stray board_periscope task behind
    # from an earlier failed run; keep the cleanup unconditional and cheap.
    await db_session.execute(
        update(TaskTable)
        .where(TaskTable.source == PERISCOPE_SOURCE)
...
@pytest.mark.asyncio
async def test_list_briefs_empty_when_none_filed(ceo_client: AsyncClient) -> None:
    resp = await ceo_client.get("/api/periscope/briefs")
    assert resp.status_code == HTTPStatus.OK
    assert resp.json() == []


@pytest.mark.asyncio
async def test_list_briefs_returns_filed_brief(
    db_session: AsyncSession, ceo_client: AsyncClient
) -> None:
    task = await _seed_completed_brief(db_session)
    resp = await ceo_client.get("/api/periscope/briefs")
    assert resp.status_code == HTTPStatus.OK
    body = resp.json()
    assert len(body) == 1
    ...
    assert row["headline"] == "A rival tool shipped agentic PR review"
    assert len(row["findings"]) == 1
    assert row["findings"][0]["source_url"] == "https://example.com/x-launch"
    assert row["threats"] == ["Feature parity gap"]
    assert row["opportunities"] == ["Lean into structured findings"]
    assert row["positioning_note"] == "Emphasize the findings ledger"


@pytest.mark.asyncio
async def test_list_briefs_omits_non_terminal_exploration(
    db_session: AsyncSession, ceo_client: AsyncClient
) -> None:
    """A still-open (PENDING) exploration cycle carries no brief yet — it
    must never render as an empty/blank brief row."""
    system = await _seed_agent(db_session, AgentRole.SYSTEM, "system")
    hom = await _seed_agent(db_session, AgentRole.HEAD_MARKETING, "head-marketing")
    db_session.add(
        TaskTable(
            id=uuid4(),
            title="Periscope market-research cycle",
...
async def test_non_ceo_is_forbidden(db_session: AsyncSession) -> None:
    await _seed_completed_brief(db_session)
    app = _build_app(db_session, AgentRole.DEVELOPER, uuid4())
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get("/api/periscope/briefs")
