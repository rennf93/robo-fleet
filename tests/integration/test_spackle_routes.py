"""Spackle (Board Program) engine route coverage — CEO-only list/approve/
reject. Mirrors test_pest_control_routes.py."""
...
async def _seed_cycle(session: AsyncSession) -> tuple[TaskTable, ProjectTable]:
    ...
    task = TaskTable(
        ...
        acceptance_criteria=["propose_gap_fill() called once"],
...
def _build_app(db_session: AsyncSession, role: AgentRole, agent_id: UUID) -> FastAPI:
    app = FastAPI()
    app.include_router(spackle_router, prefix="/api/spackle")
...
async def ceo_client(db_session: AsyncSession) -> AsyncIterator[AsyncClient]:
    ...
    # so a leftover ``board_programs=["spackle"]`` opt-in from an
    ...
    # spackle unit test that asserts an exact opted-in-project set (e.g.
    # "no project opted in"). Mirrors test_pest_control_routes.py's
    # identical cleanup. Reset, not delete — a materialized BACKLOG task
    # may still hold this project's id as an FK. Unconditional — a no-op
    # for the tests here that never wrote anything.
...
async def test_list_cycles_returns_authored_cycle(
    ...
    resp = await ceo_client.get("/api/spackle/cycles")
...
async def test_approve_unknown_item_is_404(
    ...
    resp = await ceo_client.post(
        f"/api/spackle/cycles/{task.id}/items/no-such-item/approve"
    )
...
async def test_reject_item_records_reason(
    ...
    task, _project = await _seed_cycle(db_session)
    resp = await ceo_client.post(
        f"/api/spackle/cycles/{task.id}/items/item-0/reject",
    ...
    payload = markers.get_gap_fill(refreshed)
...
async def test_non_ceo_is_forbidden(db_session: AsyncSession) -> None:
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        list_resp = await client.get("/api/spackle/cycles")
