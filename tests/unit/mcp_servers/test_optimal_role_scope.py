...
async def test_kb_search_surfaces_gaps(monkeypatch: pytest.MonkeyPatch) -> None:
    """robofleet_kb_search includes gaps in its response when the API returns them."""
    result = await _call_tool(
        "developer",
        "robofleet_kb_search",
...
async def test_kb_search_no_gaps_no_hint(monkeypatch: pytest.MonkeyPatch) -> None:
    """robofleet_kb_search does not add a gaps hint when there are no gaps."""
    result = await _call_tool(
        "developer",
        "robofleet_kb_search",
        {"query": "all good"},
...
async def test_rag_query_surfaces_gaps(monkeypatch: pytest.MonkeyPatch) -> None:
    """robofleet_rag_query includes gaps in its response when the API returns them."""
    result = await _call_tool(
        "developer",
        "robofleet_rag_query",
        {"query": "partial results"},
...
async def test_rag_query_no_gaps_no_hint(monkeypatch: pytest.MonkeyPatch) -> None:
    """robofleet_rag_query does not add a gaps hint when there are no gaps."""
    result = await _call_tool(
        "developer",
        "robofleet_rag_query",
        {"query": "all good"},
