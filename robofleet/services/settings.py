...
and parsed by typed accessors. Code defaults in ``robofleet.config`` are the
...
from robofleet.db.tables import AgentRole, SystemSettingTable
...
# to a ``robofleet.config.Settings`` bool attribute of the same name.
...
_VALIDATORS = {
    ...
    # Dotted, not in FEATURE_FLAGS (no robofleet.config attribute to fall back
    ...
    **dict.fromkeys(_FEATURE_FLAG_KEYS, _validate_bool),
    ...
    # robofleet.services.maintenance_pause). Not a feature flag: it has no
    # robofleet.config attribute, is written by its own CEO-only route, and
...
async def apply_persisted_feature_flags(session: AsyncSession) -> list[str]:
    ...
    stored value, set the matching attribute on the ``robofleet.config.settings``
