...
RoboFleet's HTTP security layer (`fastapi-guard`; see `docs/rag/architecture/http-security-guard.md`) rejected the request. This can only happen when both `ROBOFLEET_GUARD_ENABLED=true` and `ROBOFLEET_GUARD_PASSIVE_MODE=false` (enforce mode) are set. `Settings.guard_enabled` defaults to `false` in code, but `docker-compose.yml` (the build-from-source compose) overrides it to `ROBOFLEET_GUARD_ENABLED:-true` / `ROBOFLEET_GUARD_PASSIVE_MODE:-false` (enforce) - so a request from that deploy is actually blocked, not just logged. `docker-compose.registry.yml` (the pull-and-run compose) omits the guard trio, leaving the code default (off) for a third-party deploy.
...
- Prompt-injection detection  -  phrasing like "ignore previous instructions", or "bypass/disable/override the guard/filter/restriction" (a real risk in this repo's own security-work commit messages and notes: `robofleet/security.py`'s own doctring vocabulary uses "guard", "bypass", "disable" constantly)
- Secret-exfil detection  -  literal credential-shaped strings (`sk-ant-...`, `ghp_...`, postgres connection URLs) **or the literal doc pattern `ROBOFLEET_ENCRYPTION_KEY=<...>` / `ROBOFLEET_AGENT_AUTH_SECRET=<...>` / `FERNET_KEY=<...>`** (matches `CLAUDE.md` and `.env.example`'s own env-var documentation verbatim  -  quoting or editing those lines in a `note`/`commit` body trips this) or phrasing like "reveal your api keys"
- Internal-SSRF detection  -  fetch-type bodies targeting internal or metadata hosts (`169.254.169.254`, `robofleet-*` internal service hostnames)
...
2. When discussing an env var like `ROBOFLEET_ENCRYPTION_KEY` in a note/commit body, don't write it as `NAME=value` (even a placeholder value)  -  write `ROBOFLEET_ENCRYPTION_KEY` and describe the value separately, e.g. "set to a generated Fernet key", to avoid the `NAME=<10+ chars>` credential-shape match.
...
4. Never aim a fetch/HTTP-call body at an internal service host (`robofleet-*`) or a cloud metadata endpoint (`169.254.169.254`).
...
`ROBOFLEET_GUARD_ENABLED` is off by default in config; `docker-compose.yml` arms it ON with `ROBOFLEET_GUARD_PASSIVE_MODE=false` (enforce). A matching request on that deploy is genuinely blocked, not just logged - the rules above are a live restriction there, not just future-proofing. `docker-compose.registry.yml` and a bare config default both stay off. Verify against those two files directly before relying on this, since compose defaults are the kind of thing that drifts.
