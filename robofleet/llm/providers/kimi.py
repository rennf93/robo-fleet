...
Two things differ from the Claude Code spawn (mirrors ``CodexCliProvider``):
  1. **Auth** — the host's ``~/.kimi-code`` (subscription credential from
     ...
     :mod:`robofleet.llm.providers.kimi_cli_config`) but SYMLINKS
     ...
     :mod:`~robofleet.llm.providers.grok_auth` and
     :mod:`~robofleet.llm.providers.codex_auth`,
...
from robofleet.llm.providers._docker import container_running, stop_container
