...
| GrokCliProvider._append_grok_env | method | roboco/llm/providers/grok.py:205 | Append ROBOFLEET_AGENT_ID/MODEL/MCP_CONFIG/INITIAL_PROMPT/GROK_USAGE_FILE env to docker run |
...
| write_grok_hooks | function | roboco/llm/providers/grok_cli_config.py:276 | Install bash-guard PreToolUse JSON hook into ~/.grok/hooks (ROBOFLEET_GUARD_SKIP_GIT=1) |
...
| fable_honesty_nudge_hook_config | function | roboco/llm/providers/grok_cli_config.py:308 | Build the grok PostToolUse hooks JSON for the fable-mode honesty-nudge script; hook path env-overridable via ROBOFLEET_FABLE_HONESTY_NUDGE_HOOK |
...
| load_tool_manifest | function | roboco/agent_sdk/server.py:64 | Load /app/tool-manifest.json when ROBOFLEET_GATEWAY_ENABLED (call-time env read, never raises) |
...
| Name | File | Trigger |
|---|---|---|
| agent_sdk.server __main__ | roboco/agent_sdk/server.py | python -m roboco.agent_sdk.server — uvicorn FastAPI sidecar on ROBOFLEET_SDK_PORT (9000) inside every agent container |
...
| prompt_guard __main__ | roboco/agent_sdk/prompt_guard.py | grok entrypoint scans ROBOFLEET_INITIAL_PROMPT (exit 1 on injection) |
