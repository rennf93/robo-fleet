"""DogfoodService — the CEO's per-item approve/reject glue over a held
dogfood (Board Program) cycle.

The dogfood engine opens a HELD exploration task (``board_dogfood`` source);
the Product Owner authors the friction-fix drafts onto it via
...
from robofleet.services.task import DOGFOOD_ITEM_SOURCE, DOGFOOD_SOURCE
...
class FrictionFixItemResult:
    """Outcome of an approve/reject call on one friction-fix item.
...
class DogfoodService(BaseService):
    """List / approve / reject items within the open dogfood cycle(s)."""

    service_name = "dogfood_service"

    async def list_open_cycles(self) -> list[TaskTable]:
        """Every open (non-terminal) dogfood exploration task, authored or
        not."""
        from robofleet.services.task import get_task_service

        return await get_task_service(self.session).list_open_dogfood_cycles()

    async def approve_item(
        self, task_id: UUID, item_id: str, *, created_by: UUID
    ) -> FrictionFixItemResult | None:
        """Materialize one proposed item as a BACKLOG task.

        Returns None when ``task_id`` is not an open dogfood cycle or
    ...
    async def _find_item(
        ...
        """Resolve (exploration task, friction-fixes payload, one item) or
        ...
        if task is None or task.source != DOGFOOD_SOURCE:
            return None, None, None
        stored = markers.get_friction_fixes(task)
        if stored is None:
            return None, None, None
    ...
    async def _materialize(
        ...
        if not project_participates(PROGRAMS["dogfood"], project.board_programs):
            self.log.warning(
                "dogfood: materialize skipped — project not opted in",
    ...
    async def _record_learn(
        ...
        try:
            ...
            self.log.warning("dogfood: LEARN record_decision failed (best-effort)")


def get_dogfood_service(session: AsyncSession) -> DogfoodService:
    """Construct a DogfoodService bound to ``session``."""
    return DogfoodService(session)
