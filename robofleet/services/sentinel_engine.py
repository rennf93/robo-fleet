...
Mirrors ``robofleet.services.periscope_engine.PeriscopeEngine``'s "detect ->
...
Also mirrors ``robofleet.services.pest_control_engine.PestControlEngine``'s
...
* **No master enable flag.** Armed via ``robofleet.services.board_programs.
...
from robofleet.services.board_programs import program_armed
from robofleet.services.project import get_project_service
from robofleet.services.task import SENTINEL_SOURCE, TaskCreateRequest, get_task_service
...
class SentinelEngine(BaseService):
    ...
    async def _waived_trend(self) -> list[str]:
        ...
        from robofleet.services.repositories.review_findings import STATUS_WAIVED
    ...
    async def _open_findings_by_severity(self) -> list[str]:
        ...
        from robofleet.db.tables import TaskReviewFindingTable
        from robofleet.services.repositories.review_findings import STATUS_OPEN
    ...
    async def _conventions_hotspots(self) -> list[str]:
        ...
        from robofleet.db.tables import ProjectConventionFindingTable
    ...
    async def _top_task_spend(self) -> list[str]:
        ...
        from robofleet.db.tables import AgentSpawnSessionTable, TaskTable
    ...
    async def _top_project_spend(self) -> list[str]:
        ...
        from robofleet.db.tables import AgentSpawnSessionTable, ProjectTable, TaskTable
    ...
    async def _latency_section(self) -> list[str]:
        ...
        blocks the spawn."""
        from robofleet.foundation.policy.flow_timeouts import SLOW_VERBS
