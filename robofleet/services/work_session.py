...
from robofleet.db.tables import ProjectTable, TaskTable, WorkSessionTable
...
class WorkSessionService(BaseService):
    ...
    ``robofleet.templates.git.branch.build_branch_name``, which uses ``--``
    separators to avoid git ref conflicts in nested task hierarchies.
    """
