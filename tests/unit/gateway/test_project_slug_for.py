...
async def test_slug_via_project_id_unchanged() -> None:
    ...
    with patch("robofleet.services.project.get_project_service") as gps:
...
async def test_slug_falls_back_to_product_when_no_project_id() -> None:
    ...
    with (
        patch("robofleet.services.project.get_project_service") as gps,
        patch("robofleet.services.product.get_product_service") as gprod,
    ):
        gps.return_value.get = AsyncMock(return_value=proj)
...
async def test_slug_none_when_product_has_no_projects() -> None:
    c = _make_choreographer()
    with patch("robofleet.services.product.get_product_service") as gprod:
