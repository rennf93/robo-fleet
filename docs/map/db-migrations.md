...
| `VerbLatencySampleTable` | class | tables.py:~2698 | Per-verb HTTP duration samples persisted from `RequestLoggingMiddleware` for p50/p95 aggregation by `MetricsService.get_verb_latency_stats`. Migration 095; indexes on `verb`, `created_at`, composite `(verb, created_at)`. |
...
| 058 | 058_cloud_auth_users.py | `users` table (FastAPI Users schema) — the single seeded CEO login for cloud auth (`ROBOFLEET_CLOUD_AUTH_ENABLED`, default off). |
| 059 | 059_x_credentials.py | `x_credentials` (singleton Fernet-encrypted OAuth 1.0a secrets) + `x_seen_mentions` (mentions-poll dedup ledger) — the X (Twitter) engine (`ROBOFLEET_X_ENGINE_ENABLED`, default off). |
...
## Config Flags
- `ROBOFLEET_DATABASE_*` (host/port/user/password/name) — `settings.database_url`.
- `ROBOFLEET_DATABASE_ECHO`, pool size/timeout/recycle.
- No DB-specific feature flag; migrations always run. Feature flags (`ROBOFLEET_CONVENTIONS_ENABLED`, `ROBOFLEET_CI_WATCH_ENABLED`, `ROBOFLEET_DEP_UPDATE_ENABLED`, `ROBOFLEET_RELEASE_MANAGER_ENABLED`, `ROBOFLEET_ORG_MEMORY_ENABLED`) gate *use* of tables the migrations already added.
