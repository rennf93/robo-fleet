...
You do **not** call any tool to create a PR. There is no `robofleet_git_create_pr` tool of any kind.
...
Generated from templates in `robofleet/templates/git/pr_internal.py` and `robofleet/templates/git/pr_root.py`. You don't write the body by hand  -  it's filled with task title, acceptance criteria, the dev's notes, and the standard traceability links.

Title format (`build_pr_title_internal`): `[{task_id:8}] {task title}` for a leaf/internal PR. Title format (`build_pr_title_root`): `[{TaskType}] {root task title}` for an assembled root PR (e.g. `[Code] Add rate limiting`).
...
There is no `robofleet_git_merge_pr` tool of any kind.
