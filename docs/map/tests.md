...
| tests/unit/ | ~430 unit tests mirroring robofleet/: agents, api (+routes/+schemas), billing, config, conventions, db, enforcement, events, foundation/policy, gateway (105), llm, mcp_servers, migrations, models, runtime (64), scripts, services (120), templates, utils |  |
...
**Live-forge contract suites** (`test_gitea_live.py` / `test_gitlab_live.py`) are a different animal from the scripted-agent harness above — no `E2EStack`, no orchestrator, no manifests. Each drives its `GitProvider` implementation directly against a REAL forge instance (a dockerized `gitea/gitea` or real `gitlab.com`), fully self-seeding (creates its own repo/project, pushes real commits via subprocess `git`, tears down after). Both are `pytest.mark.skipif`'d off unless their pair of `ROBOFLEET_{GITEA,GITLAB}_E2E_{URL,TOKEN}` env vars is set, so a normal CI/local run never attempts network I/O against a real forge. These are what caught the slash-encoding (branch names with `/` need `quote()`-encoding before hitting Gitea's router) and the http-vs-https scheme gaps that the mocked-transport unit tests in `tests/unit/services/forge/` could not have found, since those mock the HTTP layer entirely.
...
- `tests/e2e_smoke/harness.py` clears any host-supplied `ROBOFLEET_AGENT_TOKEN` from the environment before a `ScriptedAgent` re-imports `robofleet.mcp.flow_server`. The host agent may carry a real token for the test runner's identity; `flow_server` reads it before each call and forwards it in `X-Agent-Token`. That token does not match the ephemeral test agent IDs and causes `401 Unauthorized`, so `ScriptedAgent._module` pops it to keep scripted agents in unsigned-token mode. This guard was accidentally dropped in an earlier auditor-revival commit and restored by PR #502.
...
|---|---|---|---|
| _TEST_DB_HOST/_TEST_DB_PORT/_TEST_DB_USER/_TEST_DB_PASSWORD/_TEST_DB_ADMIN_DB | module constants | tests/conftest.py:84 | Env-overridable Postgres test endpoint discovery; defaults host=localhost port=15432 user=robo-fleet password=robo-fleet admin-db=postgres (changed from :5432/$USER since baseline) |
...
flowchart TD
    ...
    F --> F1["asyncpg admin: CREATE DB robofleet_test_pid_rand"]
    ...
    subgraph tiers["Test tiers"]
      ...
      TU["tests/unit (~430) — mirror robofleet/"]
    end
    T --> tiers
...
│   ├── conftest.py  [collection gate: skips unless ROBOFLEET_E2E_SMOKE=1]
...
## Dependencies
- Internal: robofleet.db.tables (AgentTable, AuditLogTable, JournalEntryTable, JournalTable, ProjectTable, TaskTable), robofleet.db.base.Base (metadata.create_all), robofleet.models.base (AgentRole, AgentStatus, TaskStatus, TaskType, Team, Complexity, JournalEntryType, TaskNature), robofleet.foundation.policy.lifecycle (VALID_TRANSITIONS, get_valid_transitions, is_terminal_state, can_invoke_intent, intents_for_role, _INTENT_VERBS), robofleet.foundation.policy.tracing (VERB_REQUIREMENTS, VERBS_WITHOUT_TRACING), robofleet.foundation.identity/robofleet.foundation.AGENTS, robofleet.enforcement.task_lifecycle (shim), robofleet.agents_config (_BOARD_ROLES, TASK_CREATOR_ROLES, AGENT_ROLE_MAP, AGENT_TEAM_MAP), robofleet.runtime.orchestrator.AgentOrchestrator, robofleet.api.deps/robofleet.api.routes.v1.*/robofleet.services.gateway.envelope.Envelope, scripts/verify_postgres_enums.py (loaded via importlib.util), docker/scripts/bash-guard-hook.sh + docker/scripts/stop-hook.sh (shell smoke targets)
...
| Name | File | Trigger |
|---|---|---|
| make quality | Makefile | developer/CI merge gate — runs ruff format-check + ruff check + mypy robofleet/ tests/ + pytest -q --cov=robo-fleet --cov-report=term-missing --cov-fail-under=80 + xenon + vulture |
| make quality-fast | Makefile | pre-submit fast gate — ruff + mypy + pytest -q -x --no-cov (no coverage threshold) |
| make e2e-smoke | Makefile | scripted-agent lifecycle smoke — `ROBOFLEET_E2E_SMOKE=1 uv run pytest tests/e2e_smoke -q --no-cov`; needs test Postgres + git on PATH |
| make gate (panel-gate) | Makefile | panel pnpm lint + typecheck + vitest |
| make test/test-3.10..3.14/test-all | Makefile | docker compose run robo-fleet pytest -v --cov=. across Python versions |
| uv run pytest | pyproject.toml | direct invocation — auto-applies addopts (--cov=robo-fleet --cov-report=term-missing), testpaths=tests, asyncio_mode=auto |
...
## Config Flags
- ROBOFLEET_TEST_DB_HOST (default localhost)
- ROBOFLEET_TEST_DB_PORT (default 15432; was 5432 at baseline)
- ROBOFLEET_TEST_DB_USER (default robo-fleet; was $USER at baseline)
- ROBOFLEET_TEST_DB_PASSWORD (default robo-fleet; was empty at baseline)
- ROBOFLEET_TEST_DB_ADMIN_DB (default postgres)
- ROBOFLEET_AGENT_AUTH_SECRET/ROBOFLEET_AGENT_AUTH_REQUIRED/ROBOFLEET_AGENT_ID/ROBOFLEET_AGENT_ROLE/ROBOFLEET_AGENT_TOKEN/ROBOFLEET_AGENT_SESSION_ID/ROBOFLEET_AGENT_MODEL (gateway auth context set by tests via monkeypatch)
- ROBOFLEET_TOOL_MANIFEST_PATH/ROBOFLEET_ORCHESTRATOR_URL/ROBOFLEET_API_URL/ROBOFLEET_SDK_URL/ROBOFLEET_PUBLIC_BASE_URL/ROBOFLEET_MCP_CONFIG (agent-container env stubbed in unit tests)
- ROBOFLEET_GROK_REASONING_EFFORT/ROBOFLEET_GROK_TURN_TIMEOUT_SECONDS/ROBOFLEET_GROK_USAGE_FILE/ROBOFLEET_GROK_RUN_LOG/ROBOFLEET_GROK_RUN_CWD (grok provider tests)
- ROBOFLEET_GATEWAY_ENABLED/ROBOFLEET_GUARD_SKIP_GIT (gateway/guard toggles exercised in runtime+gateway tests)
- ROBOFLEET_CI_WATCH_ENABLED (tests/unit/config/test_ci_watch_flag.py + integration/services/test_ci_watch_*)
- ROBOFLEET_DEP_UPDATE_ENABLED (tests/unit/config/test_dep_update_flag.py + integration/services/test_dep_update_*)
- ROBOFLEET_CONVENTIONS_ENABLED (tests/unit/config/test_conventions_flag.py + conventions/* + gateway conventions-gate tests)
- ROBOFLEET_ORG_MEMORY_ENABLED/ROBOFLEET_ORG_MEMORY_TOP_K (tests/unit/config/test_org_memory_flag.py + gateway/test_briefing_memory.py)
- ROBOFLEET_RELEASE_MANAGER_ENABLED/ROBOFLEET_RELEASE_MIN_COMMITS (tests/unit/config/test_release_manager_flag.py + release_* service/runtime tests)
- ROBOFLEET_TOOLCHAIN_MATCH_ENABLED (tests/unit/test_toolchain_flag.py + runtime/test_toolchain_guard.py)
- ROBOFLEET_AGENT_LOOP_THRESHOLD/ROBOFLEET_AGENT_LOOP_WINDOW/ROBOFLEET_AGENT_TOOL_CALL_HALT/ROBOFLEET_AGENT_TOOL_CALL_WARN (agent_loop unit tests)
- ROBOFLEET_TRANSCRIPT_DIR/ROBOFLEET_LOG_DIR/ROBOFLEET_DATA_DIR/ROBOFLEET_WORKSPACE/ROBOFLEET_ENCRYPTION_KEY/ROBOFLEET_PROMPTER_SESSION_ID/ROBOFLEET_SECRETARY_SESSION_ID/ROBOFLEET_INITIAL_PROMPT/ROBOFLEET_GIT_AGENT/ROBOFLEET_TEST_MARKER (misc env stubs)
...
- _test_database_url force-terminates straggler backends (pg_terminate_backend) before DROP DATABASE — needed because asyncpg pool connections can outlive the engine.dispose() race; a flaky DROP could leak `robofleet_test_<pid>_<rand>` DBs.
...
- The omit list excludes robofleet/runtime/orchestrator.py, git.py, workspace.py, mcp/*, agents/*, optimal.py from the 80% gate — these are covered by NAS smoke runs, NOT unit coverage. A regression in those modules will not fail make quality.
...
- conftest default port 15432 points at Docker's robofleet-postgres (down on a bare-metal dev box). Per project memory the local-PG workflow is ROBOFLEET_TEST_DB_PORT=55432 ROBOFLEET_TEST_DB_USER=renzof ROBOFLEET_TEST_DB_PASSWORD= — without these env vars, ALL db_session tests silently skip on a non-Docker dev machine.
...
- `tests/e2e_smoke/harness.py` clears host `ROBOFLEET_AGENT_TOKEN` before scripted agents load `flow_server`. A `ScriptedAgent` pops `ROBOFLEET_AGENT_TOKEN` from `os.environ` before re-importing `robofleet.mcp.flow_server`. Without this, a real host token is forwarded for an ephemeral test agent identity and causes 401s. Any new harness path that imports `flow_server` inside a scripted agent must do the same.


## Drift from CLAUDE.md
- CLAUDE.md MEMORY note says 'conftest defaults robo-fleet@localhost:15432' and that :5432 has the robo-fleet role but NO pgvector, and :55432 is the local test PG. The conftest.py code (line 84-87) defaults to host=localhost port=15432 user=robo-fleet password=robo-fleet — matches the Docker-default claim, but CLAUDE.md's body text never documents the ROBOFLEET_TEST_DB_* env knobs or the create_all-vs-alembic drift; that lives only in the conftest docstring.
...
| Host `ROBOFLEET_AGENT_TOKEN` can leak into scripted-agent calls | tests/e2e_smoke/harness.py:476 | The harness clears the token before loading `flow_server`, but any new scripted-agent bootstrap path that forgets this step will forward the test runner's real token and get 401s for ephemeral agent IDs. | low |
