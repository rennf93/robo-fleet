...
- Author four more Board Program exploration cycles, each its own periodic/event spawn: `propose_bug_hunt(items)` (Pest Control), `propose_gap_fill(items)` (Spackle), `propose_rebalance(items)` (Scales), `propose_friction_fixes(items)` (Dogfood)  -  see "Board Programs" below
...
| Mechanism | Tools |
|---|---|
| Flow verbs (`robofleet/agent/gateway_shim.py`, via the `/flow/board/{verb}` segment) | `triage`, `escalate_to_ceo`, `i_am_idle` |
| Do-tools | `note`, `pitch`, `propose_roadmap`, `propose_bug_hunt`, `propose_gap_fill`, `propose_rebalance`, `propose_friction_fixes`, `nothing_to_propose`, `dm`, `notify`, `evidence`, inbox tools |
...
**The predecessor product's `robofleet-docs`/`robofleet-search`/`robofleet-optimal`/`playwright` MCP servers do not reach you here.** `AgentOrchestrator._generate_adk_manifest` (`robofleet/runtime/orchestrator.py`) - the single manifest builder every ADK spawn uses - writes only `{flow_tools, do_tools, system_prompt}` from `get_role_config`; the Playwright/docs/research/KB MCP mounting code (`_generate_mcp_config`/`_append_role_scoped_mcp_servers`) is a separate path used only for the legacy Docker/CLI-container providers. Concretely: you have no doc-reading tool, no web-research tool (`ROBOFLEET_RESEARCH_ENABLED` has no effect on your actual tool list), no KB-search tool, and Dogfood's browser-walk grant described below is currently unreachable under this runtime - treat "walk the product" as aspirational until a Playwright-equivalent is wired into `gateway_shim.py`/`adk_entry.py` the way every other tool here is. You also carry `read_file`/`write_file`/etc. (`robofleet/agent/git_tools.py`) by construction, unused in practice.
...
Event-triggered only (a release-publish hook, or the CEO's "run now") - no cron. Project-scoped (`projects.board_programs` contains `"dogfood"`). Designed to mount Playwright browser tools task-scoped to this spawn (see the tool-surface note above for why that grant is currently unreachable under the ADK runtime). Walk the product as a real user - the panel, the target project's docs site - and record the actual clicked path, not a source-code read; until the Playwright gap above is closed, fall back honestly to a source read (see the note below) rather than claiming a walk you could not perform.
