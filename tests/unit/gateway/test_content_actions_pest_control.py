"""robofleet.services.gateway.content_actions.propose_bug_hunt — PO-gated
Pest Control bug-hunt authoring. Mirrors test_content_actions_roadmap.py."""
...
class _FakeTask:
    ...
    ``markers`` and ``propose_bug_hunt`` touch."""
...
def _valid_item(idx: int) -> dict[str, Any]:
    return {
        ...
        "team": "backend",
        "priority": 2,
        "evidence": f"task_review_findings row F-abc{idx} recurring on this file",
    }
...
def _default_project_lookup(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    propose_bug_hunt call. Every test predating that check builds its
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_forbidden_for_non_po() -> None:
    env = await _actions("head_marketing").propose_bug_hunt(
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_forbidden_for_developer() -> None:
    env = await _actions("developer").propose_bug_hunt(
        agent_id=uuid4(), items=_valid_items(2)
    )
    assert env.error == "not_authorized"
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_empty_items() -> None:
    env = await _actions("product_owner").propose_bug_hunt(agent_id=uuid4(), items=[])
    assert env.error == "invalid_state"


@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_too_many_items() -> None:
    env = await _actions("product_owner").propose_bug_hunt(
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_missing_evidence() -> None:
    bad = _valid_item(0)
    del bad["evidence"]
    env = await _actions("product_owner").propose_bug_hunt(
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_oversized_evidence() -> None:
    bad = _valid_item(0)
    bad["evidence"] = "x" * 2001
    env = await _actions("product_owner").propose_bug_hunt(
        agent_id=uuid4(), items=[bad]
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_too_short_evidence() -> None:
    """Below the 20-char anti-soup floor (``_PEST_HUNT_ITEM_TEXT_FIELDS``) —
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_placeholder_soup_evidence() -> None:
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_missing_field() -> None:
    bad = _valid_item(0)
    del bad["description"]
    env = await _actions("product_owner").propose_bug_hunt(
        agent_id=uuid4(), items=[bad]
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_unknown_team() -> None:
    bad = _valid_item(0)
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_no_open_cycle_is_invalid_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    task_svc = MagicMock()
    task_svc.list_open_pest_control_cycles = AsyncMock(return_value=[])
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_persists_hunt_onto_open_task(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    task_svc = MagicMock()
    task_svc.list_open_pest_control_cycles = AsyncMock(return_value=[cycle_task])
    monkeypatch.setattr("robofleet.services.task.get_task_service", lambda _s: task_svc)
    actions = _actions("product_owner")
    actions.task.session.flush = AsyncMock()

    items = _valid_items(3)
    env = await actions.propose_bug_hunt(agent_id=agent_id, items=items)
    assert env.error is None
    assert env.status == "pest_hunt_proposed"
    assert env.task_id == str(cycle_task.id)

    payload = markers.get_pest_hunt(cycle_task)
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_sends_telegram_push_per_item(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    for i, call in enumerate(notify.notify_ceo_of_queue_item.await_args_list):
        assert call.kwargs["kind"] == "pest_control"
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_survives_telegram_push_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    env = await actions.propose_bug_hunt(agent_id=agent_id, items=_valid_items(1))
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_ignores_cycle_assigned_to_another_agent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    other_agent = uuid4()
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_rejects_item_targeting_unopted_project(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The positive-gate mirror of propose_roadmap's exclusion check: an item
    targeting a project that has NOT opted into pest_control is refused at
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_allows_opted_project_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    opted_project = MagicMock(board_programs=["pest_control"])
    project_svc = MagicMock()
    ...
    env = await actions.propose_bug_hunt(agent_id=agent_id, items=[bad])
    assert env.error is None


@pytest.mark.asyncio
async def test_propose_bug_hunt_allows_unresolvable_project_slug_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_propose_bug_hunt_ignores_already_authored_cycle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    authored_task = _FakeTask(
        assigned_to=agent_id,
        orchestration_markers={"pest_hunt": {"items": []}},
    )
    task_svc = MagicMock()
    task_svc.list_open_pest_control_cycles = AsyncMock(return_value=[authored_task])
    monkeypatch.setattr("robofleet.services.task.get_task_service", lambda _s: task_svc)
    env = await _actions("product_owner").propose_bug_hunt(
