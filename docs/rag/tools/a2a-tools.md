...
A2A is direct peer-to-peer messaging between agents. There is **no** `robofleet_agent_*` or `robofleet_a2a_*` tool  -  A2A is the `dm` do-tool, with `read_a2a` for reading what you were sent. There is no MCP server behind either: both are ADK `FunctionTool`s that HTTP POST to `/api/v1/do/{tool}` (`robofleet/agent/gateway_shim.py`)  -  see `docs/rag/README.md` for why this runtime has no MCP servers at all.
