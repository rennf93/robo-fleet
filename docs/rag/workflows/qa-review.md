...
# 3. Inspect the change  -  there is no git_diff/git_log tool under this
#    runtime. Work from claim_review's inline pr_url/commits/files_changed
#    plus read_file() over the checked-out worktree.
read_file(rel_path="robofleet/api/routes/rate_limit.py")

# 4. You have no shell tool, so there is no local test/lint/type-check
#    suite to run yourself  -  lean on the PR's own CI result.
...
There is no `robofleet_task_claim / _start / _qa_pass / _qa_fail` tool and no checkout tool of any kind. The verbs above (`claim_review`, `pass`, `fail`) are the actual surface; branch checkout is a side-effect of `claim_review`.
