"""Container entrypoint for the Gemini/ADK intake (prompter) agent.

The Gemini analogue of ``grok_intake_main``: the same in-container ``POST /turn``
receiver and the same relay sink to ``/api/prompter/live/{id}/events``, but the
held-open session is a :class:`GeminiChatSession` (a persistent ADK Runner over
Gemini 3.5 Flash) instead of a per-turn grok CLI invocation. The intake agent's
``propose_draft`` / ``propose_batch`` / ``search_past_tasks`` tools are wired as
the ``robofleet-intake`` MCP server via an ADK ``McpToolset`` (stdio subprocess).
The ``IntakeDriver`` loop, message source, and relay are reused unchanged from
the shared transport - only the ``SessionFactory`` differs.
"""
...
async def main() -> None:  # pragma: no cover - needs the live container + API key
    ...
    logger.info("Gemini intake container starting", session_id=session_id)
    try:
