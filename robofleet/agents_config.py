...
2. API Layer (robofleet/services/permissions.py):
...
from robofleet.infra import secretmanager as _secretmanager
from robofleet.models.base import NotificationPriority, NotificationType
from robofleet.seeds.initial_data import AGENT_UUIDS, CEO_AGENT_ID
...
def _auth_secret() -> bytes | None:
    ...
    ``gcp_secret_manager_prefix`` field defaults to ``robofleet`` and is only a
...
# Agent catalog data is canonicalized in robofleet/foundation/identity.py.
