"""Pest Control exploration dispatch — Product-Owner-solo, never the
two-reviewer board-review gate, never the dev/PM delivery dispatchers.
Mirrors test_roadmap_dispatch.py.
"""
...
from robofleet.services.task import PEST_CONTROL_SOURCE
...
@pytest.mark.asyncio
async def test_pest_control_dispatch_spawns_only_product_owner() -> None:
    """A pest-control exploration task must spawn the Product Owner alone."""
    orch = _make_orch()
...
@pytest.mark.asyncio
async def test_pest_control_dispatch_skips_once_authored() -> None:
    """Once ``propose_bug_hunt`` has stamped the pest_hunt marker, the
    dispatcher must not spawn again."""
    orch = _make_orch()
    task = _pest_control_task(orchestration_markers={"pest_hunt": {"items": []}})
    with (
        patch.object(orch, "_is_agent_active", return_value=False),
        patch.object(orch, "spawn_agent", new=AsyncMock()) as spawn,
    ):
...
@pytest.mark.asyncio
async def test_pest_control_dispatch_skips_active_po() -> None:
    orch = _make_orch()
    task = _pest_control_task()
    with (
...
@pytest.mark.asyncio
async def test_dispatch_pm_work_routes_pest_control_away_from_board_handler() -> None:
    """A board_pest_control task must ride the dedicated dispatcher, never
    the two-reviewer ``_handle_board_assigned_task``."""
    task = _pest_control_task()
    stub = MagicMock()
    ...
    stub._dispatch_pest_control_exploration.assert_awaited_once()
    stub._handle_board_assigned_task.assert_not_awaited()
    stub._handle_pm_assigned_task.assert_not_awaited()


@pytest.mark.asyncio
async def test_pest_control_tasks_are_never_routed_by_dev_dispatch() -> None:
    tasks = [_pest_control_task()]
    stub = MagicMock()
...
def test_pest_control_prompt_names_solo_po_and_real_verbs() -> None:
    orch = _make_orch()
    prompt = orch._build_pest_control_prompt(_pest_control_task())
    assert "triage()" in prompt
    assert "propose_bug_hunt(" in prompt
    assert "i_am_idle()" in prompt
    assert "do not" in prompt.lower()


def test_pest_control_prompt_omits_optional_sections_when_empty() -> None:
    orch = _make_orch()
    prompt = orch._build_pest_control_prompt(_pest_control_task())
...
def test_pest_control_prompt_renders_prior_cycles_and_evidence_when_given() -> None:
    orch = _make_orch()
    prompt = orch._build_pest_control_prompt(
        _pest_control_task(),
        "proposed 2, approved 1",
        "Rework hotspots (revision_count >= 2):\n- Chronic task — bounced 3x",
    )
    assert "## Prior cycles" in prompt
    assert "proposed 2, approved 1" in prompt
    assert "## Evidence gathered for you" in prompt
    assert "Chronic task — bounced 3x" in prompt


@pytest.mark.asyncio
async def test_pest_control_dispatch_injects_prior_context_and_evidence() -> None:
    orch = _make_orch()
    task = _pest_control_task()
    with (
        patch.object(orch, "_is_agent_active", return_value=False),
        ...
        patch.object(
            orch,
            "_pest_control_evidence_context",
            AsyncMock(return_value="- Chronic task — bounced 3x"),
        ),
        patch.object(orch, "spawn_agent", new=AsyncMock()) as spawn,
    ):
        await orch._dispatch_pest_control_exploration(task)
...
@pytest.mark.asyncio
async def test_pest_control_evidence_context_survives_db_failure() -> None:
    """A DB hiccup gathering evidence degrades to '' — never raises."""
    orch = _make_orch()
    with patch(
        "robofleet.services.pest_control_engine.get_pest_control_engine",
        side_effect=RuntimeError("db down"),
    ):
        result = await orch._pest_control_evidence_context()
    assert result == ""
