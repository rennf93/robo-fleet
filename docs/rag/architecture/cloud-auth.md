...
A second route mints this same session cookie without a password: `POST /api/telegram/webapp-auth` validates a Telegram Mini App's signed `initData` instead, gated by its own `ROBOFLEET_TELEGRAM_MINIAPP_ENABLED` (which itself requires `cloud_auth_enabled`)  -  see `the API route modules under `robofleet/api/routes/``.

## Related

- `docs/rag/architecture/config-reference.md`  -  full env var table
- `ROBOFLEET_DB_NETWORK_ISOLATED` (`robofleet/config.py`)  -  a separate, unrelated hardening (network topology, not auth) that can ship alongside cloud auth on a given compose file
