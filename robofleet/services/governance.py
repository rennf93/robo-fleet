...
class GovernanceService:
    ...
    async def get_report(self, task_id: UUID) -> TaskGovernanceReportResponse | None:
        ...
        from robofleet.api.schemas.tasks import TaskGovernanceReportResponse
    ...
    async def _build_gate_chain(
        ...
        gate_events = self._collect_gate_events(events)
        ...
        from robofleet.api.schemas.tasks import GateStageResponse
    ...
    @staticmethod
    def _collect_gate_events(
        events: list[AuditLogTable],
    ) -> dict[str, tuple[str, datetime | None]]:
        """Map each gate to its latest audit event (passed/failed). Bounce
        events (qa_fail, pr_fail, request_changes, ceo_reject) mark the gate
        as failed; advancement events mark it as passed."""
    ...
    async def _findings_summary(self, task_id: UUID) -> list[TaskFindingsSummaryRow]:
        ...
        from robofleet.api.schemas.tasks import findings_summary
