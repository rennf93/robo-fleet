...
| `robofleet_git_status(project_slug)` / `robofleet_git_log(project_slug, limit?, branch?)` / `robofleet_git_diff(project_slug, branch?, base?)` / `robofleet_git_branches(project_slug)` | Read-only git inspection. Use these (not raw `Bash git ...`) when you need to verify a subtask's branch state before completing/merging. | None. |
...
- `intends_to_touch` — the files/dirs this subtask will modify (globs are fine, e.g. `["robofleet/services/git.py", "robofleet/api/routes/**"]`). Read the brief and the code you can see; name the real paths.
