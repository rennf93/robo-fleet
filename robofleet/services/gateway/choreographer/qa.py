...
# (robofleet.foundation.policy.content.models._FINDING_FIX_CAP): a pointer
...
class QAMixin(_Base):
    ...
    async def claim_review(self, qa_agent_id: UUID, task_id: UUID) -> Envelope:
        ...
        # the retry would re-race for the task. With the commit, a cancelled
        # request leaves the claim standing and the retry resumes into an
        ...
        if to_python_uuid(t.active_claimant_id) != qa_agent_id:
            ...
            if claimed is None:
                return await self._emit_rejection(
                    Envelope.not_authorized(
                        message=(
                            "this review task is already claimed by another agent"
                        ),
