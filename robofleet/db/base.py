...
from robofleet.infra.cloudsql import async_engine_for_cloudsql
...
async def get_db() -> AsyncGenerator[AsyncSession]:
    ...
    on ``get_db_committed`` instead (``robofleet.api.deps.DbSession`` — the one
...
async def get_db_committed(
    ...
    (``robofleet/api/middleware.py``) can commit it BEFORE the response reaches
    the client.
