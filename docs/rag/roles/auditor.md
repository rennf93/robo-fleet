...
| Mechanism | Tools |
|---|---|
| Flow verbs (`robofleet/agent/gateway_shim.py`) | `triage`, `waive_finding`, `i_am_idle` |
| Do-tools | `note` (scope=`reflect`), `evidence`, `dm`, `read_a2a` (reply-only, see below), `notify_list`, `notify_get`, `approve_playbook`, `reject_playbook`, `archive_playbook`, `curate_vault`, `propose_postmortem`, `propose_playbook_drafts`, `propose_quality_report`, `nothing_to_propose` |

You carry **no git/file tools at all** - `robofleet.agent.adk_entry` builds `build_git_tools()` unconditionally for every role, but nothing in your manifest ever gives you a reason to call them; you have no `commit`, `notify`, or any task-mutating write verb. `curate_vault` is the one write you make, and it targets an Obsidian vault note, never task state, code, or git.
