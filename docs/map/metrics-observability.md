...
| `robofleet/api/middleware.py` | `RequestLoggingMiddleware` - the single timing seam for every HTTP request; now also the per-verb latency telemetry persistence point via `_extract_flow_verb_role` + `_persist_verb_latency` (fire-and-forget `asyncio.create_task` on a separate session). `FlowVerbTimeoutMiddleware` owns the 504-timeout path whose status code the telemetry classifies as `outcome="timeout"`. | ~700 |
...
| `VerbLatencySampleTable` | class | db/tables.py | `verb_latency_samples` table - one row per gateway-verb invocation persisted from `RequestLoggingMiddleware` for p50/p95 aggregation. Columns: `id` (UUID PK), `verb` (String 80, indexed), `role` (String 50), `duration_ms` (Float), `outcome` (String 20: success/error/timeout), `status_code` (Integer nullable), `created_at` (DateTime tz, indexed). Composite index `(verb, created_at)` backs the windowed aggregation query. Migration 095. |
| `_extract_flow_verb_role` | func | api/middleware.py | Parses `(role, verb)` from a `/api/v1/flow/{role}/{verb}` request path; returns `None` for non-flow paths so telemetry only persists for gateway verbs. Uses `path.rstrip("/").rsplit("/", 2)`. |
| `_persist_verb_latency` | func | api/middleware.py | Fire-and-forget write of one `VerbLatencySampleTable` row using a **separate async session** from `get_session_factory()` (not the request's own session, which `DbCommitMiddleware` owns). Never raises - a telemetry failure is logged and swallowed so the verb itself is unaffected. Scheduled via `asyncio.create_task` so the response returns before the INSERT runs. |
| `MetricsService.get_verb_latency_stats` | method | metrics.py:~1777 | p50/p95 HTTP duration per gateway verb over a configurable `hours` window (default 24h), backed by `PERCENTILE_CONT(0.5)` / `PERCENTILE_CONT(0.95)` on `verb_latency_samples`. Returns `list[VerbLatencyStats]` sorted by p95 descending with per-verb sample counts. Follows the `get_spawn_waste_metrics` / `get_provenance_metrics` pattern (bounded window param, raw SQL via `text()`). |
| `VerbLatencyStats` | dataclass | models/metrics.py | `{verb, p50_ms, p95_ms, sample_count}` + `to_dict()` (p50/p95 rounded to 2dp). One row per verb in the aggregation result. |
| `SentinelEngine._latency_section` | method | sentinel_engine.py:~259 | Renders the slowest gateway verbs by p95 (capped at `_TREND_LIMIT` ~ 10) into `evidence_context` as a `"Per-verb latency (p50/p95)"` section. Flags any verb whose p95 crosses its matching server-side timeout: `settings.flow_verb_timeout_seconds * 1000` (120s default) for normal verbs, `settings.flow_verb_slow_timeout_seconds * 1000` (900s) for `SLOW_VERBS` from `robofleet/foundation/policy/flow_timeouts.py`. Catches DB errors and returns `[]` so a telemetry gap never blocks the spawn. |
...
A third upstream - **per-verb HTTP latency telemetry** - flows from `RequestLoggingMiddleware` (the single timing seam, app-wide) into the `verb_latency_samples` table and out to Sentinel evidence. On every `POST /api/v1/flow/{role}/{verb}` request, the middleware extracts `(role, verb)` via `_extract_flow_verb_role`, classifies the outcome (`"success"` on the success path, `"timeout"` when the response status is 504 from `FlowVerbTimeoutMiddleware`, `"error"` on the exception path), and schedules `_persist_verb_latency` as a fire-and-forget `asyncio.create_task` on a **separate async session** - the response returns before the INSERT runs, and a telemetry failure is logged and swallowed, never failing the verb. `MetricsService.get_verb_latency_stats` then reads the table with `PERCENTILE_CONT` over a configurable hours window, and `SentinelEngine._latency_section` renders the slowest verbs into the Sentinel quality report's `evidence_context`, flagging any whose p95 crosses its matching server-side timeout. No new per-verb instrumentation was added - the middleware is the single seam and already covers every verb.
...
graph LR
  ...
  FlowReq[POST /api/v1/flow/{role}/{verb}] --> Middleware[RequestLoggingMiddleware]
  Middleware -->|asyncio.create_task<br>separate session| VLS[verb_latency_samples]
  VLS --> LatencyStats[MetricsService.get_verb_latency_stats<br>PERCENTILE_CONT p50/p95]
  LatencyStats --> Sentinel[SentinelEngine._latency_section<br>evidence_context]
```
...
**Internal (robo-fleet):**
- `robofleet.db.tables` - `AgentSpawnSessionTable`, `DailyUsageRollupTable`, `AgentTable`, `AuditLogTable`, `TaskTable`, `NotificationTable`, `VerbLatencySampleTable`
- `robofleet.db.base` - `get_session_factory` (separate-session telemetry write path)
- `robofleet.api.middleware` - `RequestLoggingMiddleware` (telemetry persistence seam), `FlowVerbTimeoutMiddleware` (504-timeout classification)
- `robofleet.foundation.policy.flow_timeouts` - `SLOW_VERBS` (timeout-breach flag threshold in Sentinel latency section)
...
- **Middleware telemetry seam**: `RequestLoggingMiddleware.dispatch` (`api/middleware.py`) fires `_persist_verb_latency` via `asyncio.create_task` on every `POST /api/v1/flow/{role}/{verb}` - both the success path (status code available, outcome `"success"` or `"timeout"` on 504) and the exception path (outcome `"error"`). This is the sole write path into `verb_latency_samples`; no per-verb instrumentation exists elsewhere. `MetricsService.get_verb_latency_stats` is consumed by `SentinelEngine._latency_section` (not yet exposed as a standalone HTTP route).
...
- **Verb-latency telemetry is fire-and-forget - the task reference is not stored.** `RequestLoggingMiddleware.dispatch` schedules `_persist_verb_latency` via `asyncio.create_task` without keeping a reference to the Task object. The response returns before the INSERT runs (true non-blocking, single-digit-ms overhead on the verb call itself), but if the event loop exits mid-write (e.g. orchestrator shutdown) the in-flight sample is silently lost. This is a deliberate trade-off over `await`-ing the INSERT in a `try`/`except` (which would add 1-3ms to the response path). The `# noqa: RUF006` suppression on each `create_task` call is intentional.
- **A 504 response is classified as `outcome="timeout"`, not `"success"`.** `FlowVerbTimeoutMiddleware` sends its 504 via `send()` without re-raising, so the 504 lands on `RequestLoggingMiddleware`'s success path (a response object exists with `status_code=504`), not the exception path. The telemetry write checks `response.status_code == 504` to set `outcome="timeout"` - a future middleware that returns a 504 for a non-timeout reason would be misclassified as a timeout. The exception path (no response object) always writes `outcome="error"` with `status_code=None`.
- **Telemetry writes use a separate async session from the request's own.** `_persist_verb_latency` calls `get_session_factory()` and opens a fresh `async with factory() as session` - deliberately NOT the request's session, which is committed by `DbCommitMiddleware`. A telemetry failure (DB unavailable, connection lost) is caught in a `try`/`except Exception` that logs `verb_latency: failed to persist telemetry sample` with `exc_info=True` and swallows. This means a transient DB outage silently drops samples with only a warning log - the `verb_latency_samples` table can have gaps during a DB hiccup.
- **`PERCENTILE_CONT` uses linear interpolation, not nearest-rank.** `MetricsService.get_verb_latency_stats` uses PostgreSQL's `PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY duration_ms)`, which linearly interpolates between adjacent values. For a 4-sample set `[100, 200, 300, 400]`, p95 = 385 (rank 2.85 → 300 + 0.85×100), not 400. A test assertion expecting 400 was caught and fixed in QA round 1 (RoboFleet F-972786cb) - the implementation was correct, only the test expected the wrong value. Any future caller comparing against a discrete sample must account for interpolation, not assume the maximum.
...
>
> **Per-verb latency telemetry (2026-08-21, upstream PR #928, commit 5b09b897c).** Closes the gap that the org measured spend, waivers, findings, and rework but not the latency of its own control-plane verbs - the one number standing between the current state and the sub-24h lead-time objective. Four-part change, all backend: (1) **Persistence** - new `verb_latency_samples` table (`VerbLatencySampleTable` in `db/tables.py`, migration 095 in robo-fleet, renumbered from RoboFleet's 094) with columns `id`/`verb`/`role`/`duration_ms`/`outcome`/`status_code`/`created_at` and three indexes (verb, created_at, composite verb+created_at). `RequestLoggingMiddleware.dispatch` - the single timing seam that already computes `duration_ms` via `time.perf_counter()` on every request - now extracts `(role, verb)` from `/api/v1/flow/{role}/{verb}` paths via `_extract_flow_verb_role` and schedules `_persist_verb_latency` as a fire-and-forget `asyncio.create_task` on a **separate async session** from `get_session_factory()`. Outcome classification: `"success"` on the success path with `status_code`, `"timeout"` when the 504 comes from `FlowVerbTimeoutMiddleware` (detected via `response.status_code == 504`), `"error"` on the exception path with `status_code=None`. A telemetry failure is logged and swallowed - never fails the verb. (2) **Aggregation** - `MetricsService.get_verb_latency_stats(hours=24)` computes p50/p95 per verb via `PERCENTILE_CONT` on the new table, returning `list[VerbLatencyStats]` (new dataclass in `models/metrics.py`) sorted by p95 descending with per-verb sample counts. (3) **Sentinel integration** - `SentinelEngine._latency_section` renders the slowest verbs (capped at `_TREND_LIMIT`) into `evidence_context` as a `"Per-verb latency (p50/p95)"` section, flagging any verb whose p95 crosses its matching server-side timeout (`settings.flow_verb_timeout_seconds * 1000` for normal verbs, `settings.flow_verb_slow_timeout_seconds * 1000` for `SLOW_VERBS`). Catches DB errors and returns `[]` so a telemetry gap never blocks the spawn. (4) **Tests** - `tests/unit/api/test_verb_latency_telemetry.py` (persistence path parser, success/exception/504 classification, non-blocking guarantee) and `tests/unit/services/test_verb_latency.py` (aggregation correctness against real Postgres, Sentinel integration with timeout-breach flags). No new per-verb instrumentation - the middleware is the single seam. Not yet exposed as a standalone HTTP route; consumed only by Sentinel.
...
| Verb-latency fire-and-forget can lose samples on loop exit | api/middleware.py | `asyncio.create_task` schedules the telemetry INSERT without storing the Task reference, so an event-loop exit (orchestrator shutdown) mid-write silently drops the in-flight sample. Deliberate trade-off for sub-single-digit-ms overhead; a transient DB outage also drops samples (logged + swallowed). `verb_latency_samples` can have gaps during infrastructure hiccups. | low |
| 504-timeout classification depends on `FlowVerbTimeoutMiddleware`'s send-only 504 | api/middleware.py | The telemetry classifies `outcome="timeout"` by checking `response.status_code == 504` on the success path. A future middleware returning a 504 for a non-timeout reason would be misclassified as a timeout. The exception path (no response object) always writes `outcome="error"`. | low |
