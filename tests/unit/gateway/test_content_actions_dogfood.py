"""robofleet.services.gateway.content_actions.propose_friction_fixes — PO-gated
Dogfood friction-fix authoring. Mirrors test_content_actions_spackle.py."""
...
class _FakeTask:
    ...
    ``markers`` and ``propose_friction_fixes`` touch."""
...
def _valid_item(idx: int) -> dict[str, Any]:
    return {
        ...
        "priority": 2,
        "evidence": (
            f"Walked /settings{idx} -> clicked Save -> no toast confirming "
            "the write (network tab: 200 OK, but no UI feedback at all)"
        ),
    }
...
def _default_project_lookup(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    propose_friction_fixes call. Every test predating that check builds its
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_forbidden_for_non_po() -> None:
    env = await _actions("head_marketing").propose_friction_fixes(
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_forbidden_for_developer() -> None:
    env = await _actions("developer").propose_friction_fixes(
        agent_id=uuid4(), items=_valid_items(2)
    )
    assert env.error == "not_authorized"
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_empty_items() -> None:
    env = await _actions("product_owner").propose_friction_fixes(
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_too_many_items() -> None:
    env = await _actions("product_owner").propose_friction_fixes(
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_missing_evidence() -> None:
    bad = _valid_item(0)
    del bad["evidence"]
    env = await _actions("product_owner").propose_friction_fixes(
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_oversized_evidence() -> None:
    bad = _valid_item(0)
    bad["evidence"] = "x" * 2001
    env = await _actions("product_owner").propose_friction_fixes(
        agent_id=uuid4(), items=[bad]
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_too_short_evidence() -> None:
    """Below the 20-char anti-soup floor (``_FRICTION_FIXES_ITEM_TEXT_
    FIELDS``) — distinct from the missing-field and oversized-evidence
    checks above."""
    bad = _valid_item(0)
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_placeholder_soup_evidence() -> None:
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_missing_field() -> None:
    bad = _valid_item(0)
    del bad["description"]
    env = await _actions("product_owner").propose_friction_fixes(
        agent_id=uuid4(), items=[bad]
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_unknown_team() -> None:
    bad = _valid_item(0)
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_no_open_cycle_is_invalid_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    task_svc = MagicMock()
    task_svc.list_open_dogfood_cycles = AsyncMock(return_value=[])
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_persists_audit_onto_open_task(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    task_svc = MagicMock()
    task_svc.list_open_dogfood_cycles = AsyncMock(return_value=[cycle_task])
    monkeypatch.setattr("robofleet.services.task.get_task_service", lambda _s: task_svc)
    actions = _actions("product_owner")
    actions.task.session.flush = AsyncMock()

    items = _valid_items(3)
    env = await actions.propose_friction_fixes(agent_id=agent_id, items=items)
    assert env.error is None
    assert env.status == "friction_fixes_proposed"
    assert env.task_id == str(cycle_task.id)

    payload = markers.get_friction_fixes(cycle_task)
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_sends_telegram_push_per_item(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    for i, call in enumerate(notify.notify_ceo_of_queue_item.await_args_list):
        assert call.kwargs["kind"] == "dogfood"
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_survives_telegram_push_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    env = await actions.propose_friction_fixes(agent_id=agent_id, items=_valid_items(1))
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_ignores_cycle_assigned_to_another_agent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    other_agent = uuid4()
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_rejects_item_targeting_unopted_project(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The positive-gate mirror of propose_gap_fill's exclusion check: an item
    targeting a project that has NOT opted into dogfood is refused at
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_allows_opted_project_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    cycle_task = _FakeTask(assigned_to=agent_id)
    ...
    opted_project = MagicMock(board_programs=["dogfood"])
    project_svc = MagicMock()
    ...
    env = await actions.propose_friction_fixes(agent_id=agent_id, items=[bad])
    assert env.error is None


@pytest.mark.asyncio
async def test_propose_friction_fixes_allows_unresolvable_project_slug_through(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_propose_friction_fixes_ignores_already_authored_cycle(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent_id = uuid4()
    authored_task = _FakeTask(
        assigned_to=agent_id,
        orchestration_markers={"friction_fixes": {"items": []}},
    )
    task_svc = MagicMock()
    task_svc.list_open_dogfood_cycles = AsyncMock(return_value=[authored_task])
    monkeypatch.setattr("robofleet.services.task.get_task_service", lambda _s: task_svc)
    env = await _actions("product_owner").propose_friction_fixes(
