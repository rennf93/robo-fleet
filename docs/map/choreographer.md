...
- **`_build_qa_claim_evidence` (`claim_review`) must NOT `asyncio.gather` the git leg with the DB reads.** `self.task`/`self.git`/`self.evidence_repo` share one request-scoped `AsyncSession` (see `deps.py`), and the git leg's own workspace/token resolution (`diff_and_files` → `_resolve_diff_refs` → `_workspace_for_branch`/`_token_for_branch`) runs DB lookups against that same session — so concurrently awaiting it alongside `journal_highlights_for_task`/`ancestor_context_for_task` risks two queries racing one `AsyncSession` (unsupported by SQLAlchemy) plus a `wait_for`-cancelled DB query mid-flight if the git leg times out. The git leg is awaited on its own via `run_bounded_leg`; the two DB reads run sequentially afterward. The combined git leg's timeout gap note is labeled `"pr diff + files_changed"` (not just `"pr diff"`) so QA can tell `files_changed` is empty because of a timeout, not genuinely empty — a combined-leg timeout kills both together. The internal subprocess-level concurrency inside `diff_and_files` (the full diff and the `--name-only` diff running concurrently) is untouched and remains the real latency win.
