...
from robofleet.services.gateway.evidence_builder import render_findings
from robofleet.services.gateway.merge_chain import resolve_parent_branch
from robofleet.utils.converters import to_python_uuid
...
class PRGateMixin(_Base):
    ...
    async def _scope_relaxation_drift_guard(
        ...
        globs (or when the file fetch fails; fail-open, matching the best-effort
        posture of ``_gate_changed_files``). Returns an ``invalid_state``
        envelope naming each out-of-scope file when drift is detected, after
        recording each as a findings-ledger row so Pest Control and Sentinel can
        see scope drift accumulate.
        """
