"""Dogfood (Board Program) engine route coverage — CEO-only list/approve/
            ...
            "items": [
                {
                    ...
                    "description": "Clicking Save shows nothing — no toast, no spinner",
                    "acceptance_criteria": ["a toast confirms the save"],
                    "project_slug": project.slug,
                    "team": "frontend",
                    "priority": 2,
                    "evidence": (
                        "Walked /settings -> changed panel_base_url -> clicked "
                        "Save -> network tab shows 200 OK but no UI feedback"
                    ),
                    "status": "proposed",
                    "reject_reason": None,
...
def _build_app(db_session: AsyncSession, role: AgentRole, agent_id: UUID) -> FastAPI:
    app = FastAPI()
    app.include_router(dogfood_router, prefix="/api/dogfood")
...
async def ceo_client(db_session: AsyncSession) -> AsyncIterator[AsyncClient]:
    ...
    # so a leftover ``board_programs=["dogfood"]`` opt-in from an
    ...
    # dogfood unit test that asserts an exact opted-in-project set (e.g.
...
async def test_list_cycles_returns_authored_cycle(
    ...
    resp = await ceo_client.get("/api/dogfood/cycles")
...
async def test_approve_unknown_item_is_404(
    ...
    resp = await ceo_client.post(
        f"/api/dogfood/cycles/{task.id}/items/no-such-item/approve"
    )
...
async def test_reject_item_records_reason(
    ...
    task, _project = await _seed_cycle(db_session)
    resp = await ceo_client.post(
        f"/api/dogfood/cycles/{task.id}/items/item-0/reject",
    ...
    payload = markers.get_friction_fixes(refreshed)
...
async def test_non_ceo_is_forbidden(db_session: AsyncSession) -> None:
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        list_resp = await client.get("/api/dogfood/cycles")
