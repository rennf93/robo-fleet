...
import pytest
from robofleet.bootstrap import (
    _BootstrapHolder,
    _run_api_server,
    _wait_for_api_ready,
    main,
...
async def test_run_api_server_starts_uvicorn() -> None:
    ...
    with (
        patch("robofleet.bootstrap.uvicorn.Config") as cfg_cls,
        patch("robofleet.bootstrap.uvicorn.Server", return_value=server_instance),
    ):
...
async def test_wait_for_api_ready_returns_when_health_ok() -> None:
    ...
    with patch("robofleet.bootstrap.httpx.AsyncClient", return_value=client_instance):
...
async def test_wait_for_api_ready_swallows_errors_and_times_out() -> None:
    ...
    with (
        patch("robofleet.bootstrap.httpx.AsyncClient", return_value=client_instance),
        patch("robofleet.bootstrap.asyncio.sleep", sleep_mock),
    ):
        await _wait_for_api_ready(max_wait=4)
...
async def test_wait_for_api_ready_keeps_polling_on_non_200() -> None:
    ...
    with (
        patch("robofleet.bootstrap.httpx.AsyncClient", return_value=client_instance),
        patch("robofleet.bootstrap.asyncio.sleep", AsyncMock()),
    ):
        await _wait_for_api_ready(max_wait=4)
...
async def test_main_skip_orchestrator_returns_after_db_bootstrap() -> None:
    ...
    db_mock = AsyncMock()
    with (
        patch("robofleet.bootstrap.bootstrap_database", db_mock),
        patch("robofleet.bootstrap.init_event_bus") as bus,
    ):
...
async def test_main_full_path_starts_orchestrator_and_shuts_down() -> None:
    ...
    with (
        patch("robofleet.bootstrap.bootstrap_database", AsyncMock()),
        patch("robofleet.bootstrap.init_event_bus", AsyncMock(return_value=bus)),
        patch("robofleet.bootstrap.register_default_handlers"),
        patch("robofleet.bootstrap.start_websocket_bridge", AsyncMock()),
        patch("robofleet.bootstrap.AgentOrchestrator", return_value=orch),
        patch("robofleet.bootstrap.set_orchestrator"),
        patch("robofleet.bootstrap.NotificationService"),
        patch("robofleet.bootstrap.set_event_context"),
        patch("robofleet.bootstrap.set_reasoning_stream_callback"),
        patch("robofleet.bootstrap._wait_for_api_ready", AsyncMock()),
        ...
        # without warnings.
        patch("robofleet.bootstrap._run_api_server", AsyncMock(return_value=None)),
    ):
        await main()
...
def test_module_run_as_script_invokes_cli() -> None:
    """`python -m robofleet.bootstrap` → calls `robofleet.cli.cli()`."""
    cli_mock = MagicMock()
    with patch.dict(
        "sys.modules",
        {"robofleet.cli": MagicMock(cli=cli_mock)},
    ):
        runpy.run_module("robofleet.bootstrap", run_name="__main__")
...
async def test_main_handles_api_task_cancellation() -> None:
    ...
    with (
        ...
        patch("robofleet.bootstrap.set_reasoning_stream_callback"),
        patch("robofleet.bootstrap._wait_for_api_ready", AsyncMock()),
        patch("robofleet.bootstrap._run_api_server", _raise_cancelled),
    ):
        await main()
