...
if TYPE_CHECKING:
    ...
    from robofleet.llm.providers import AgentProvider, ProviderRegistry
    from robofleet.services.llm import AgentRoute
    from robofleet.services.maintenance_pause import PauseScope
...
from robofleet.foundation.identity import (
    CELL_TEAMS,
    is_human_only_role,
...
from robofleet.foundation.policy.agent_loop import DEFAULT_BUDGET as _AGENT_LOOP_BUDGET
from robofleet.foundation.policy.batch import is_branchless_coordination
...
from robofleet.models.sandbox import SandboxInfo
from robofleet.runtime.compose_labels import compose_label_args
from robofleet.runtime.sandbox import SandboxProvisioner
...
def _agent_api_headers(agent_uuid: str, role: str) -> dict[str, str]:
    ...
    """
    from robofleet.agents_config import _auth_secret, issue_agent_token
...
# robofleet.llm.providers.gemini_cli_usage.classify_exit_code — the CLI itself has
...
# (see robofleet.llm.providers.gemini) — so a genuinely bad/missing credential
...
def _build_manifest_for_agent(
    ...
    from robofleet.runtime.spawn_manifest import (
        SpawnInputs,
        build_for_role,
        write_manifest,
    )
...
class AgentOrchestrator:
    ...
    def _fire_audit(
        ...
        *,
        ...
        from robofleet.services.audit import get_audit_service
    ...
    async def _git_context_default_project(self) -> SpawnGitContext | None:
        ...
        from robofleet.models.env_branches import head_branch
    ...
    async def _git_context_from_task_id(self, task_id: str) -> SpawnGitContext | None:
        ...
        from robofleet.db.base import get_db_context
        from robofleet.db.tables import ProjectTable, TaskTable
    ...
    async def _prepare_agent_spawn(
        ...
        agent_id = self._safe_agent_path_segment(agent_id)
    ...
    def _ensure_provider_registry(self) -> "ProviderRegistry":
        ...
        robofleet.llm.providers.gemini for the V1 scope), and KIMI (Moonshot,
        official CLI, one-shot delivery roles only — see
        robofleet.llm.providers.kimi for the V1 scope).
        """
    ...
    async def _is_video_authoring_spawn(
        ...
        try:
            ...
            from robofleet.db.base import get_session_factory
            from robofleet.services.task import VIDEO_SOURCE, get_task_service
    ...
    async def _is_dogfood_spawn(
        ...
        try:
            ...
            from robofleet.db.base import get_session_factory
            from robofleet.services.task import DOGFOOD_SOURCE, get_task_service
    ...
    def _build_mcp_env(
        ...
        mcp_env: dict[str, str] = {
            ...
            # robofleet.mcp.<server>` by Claude Code, with cwd = the agent's
    ...
    async def _ambient_projects_for_task(db: Any, task_id: str) -> list[Any]:
        ...
        if task.product_id is not None:
            from robofleet.services.product import get_product_service
    ...
    async def _resolve_history_digest_ambient(
        ...
        try:
            from robofleet.db.base import get_session_factory
            from robofleet.services.prompter import history_digest_layer
    ...
    async def _resolve_agent_route(
        ...
        from robofleet.models.base import ModelProvider
        from robofleet.models.runtime import MODEL_MAP
    ...
    # in robofleet/agents/factories/_base.py — kept in sync because the
    ...
    def _open_intake_relay(session_id: str) -> None:
        ...
        from robofleet.services.prompter_live import get_live_registry
    ...
    async def _release_stopped_agent_claim(
        ...
        from robofleet.db.base import get_session_factory
        from robofleet.services.task import TaskService
        from robofleet.utils.converters import InvalidIdentifierError, require_uuid
    ...
    async def _persist_waiting_record(self, record: WaitingRecord) -> None:
        ...
        try:
            ...
            from robofleet.db.base import get_session_factory
            from robofleet.db.tables import WaitingRecordTable
    ...
    async def _persist_token_snapshot(
        ...
        from robofleet.db.tables import AgentSpawnSessionTable, TokenUsageSnapshotTable
    ...
    async def _sweep_token_snapshots(self) -> None:
        ...
        if _usage_by_agent:
            with contextlib.suppress(Exception):
                from robofleet.events import get_event_bus
                from robofleet.services.usage_events import (
                    UsageSnapshot,
    ...
    async def _upsert_rollup_row(self, db: Any, row: Any, uuid4: Any) -> None:
        ...
        from robofleet.db.tables import DailyUsageRollupTable
    ...
    async def restore_respawn_tracker(self) -> int:
        ...
        try:
            ...
            from robofleet.db.base import get_session_factory
            from robofleet.db.tables import RespawnTrackerTable, TaskTable
...
"""
    ...
    async def _reconcile_umbrella_row(
        ...
        """
        from robofleet.foundation.policy.content import markers as _sm
        from robofleet.services.project import get_project_service
        from robofleet.services.task import get_task_service
        from robofleet.utils.converters import require_uuid
    ...
    async def _sweep_transcript_retention(self) -> None:
        ...
        with contextlib.suppress(Exception):
            from robofleet.db.base import get_session_factory
            from robofleet.services.settings import get_settings_service
        ...
        from robofleet.runtime.transcript_retention import select_prunable_transcripts
    ...
    async def _task_budget_breach(self, task_id_str: str) -> tuple[float, float] | None:
        ...
        """
        from robofleet.db.base import get_db_context
        from robofleet.foundation.policy.agent_loop import effective_task_budget_usd
        from robofleet.models.base import TaskStatus
        from robofleet.services.task import TaskService
        from robofleet.utils.converters import InvalidIdentifierError, require_uuid
    ...
    async def _fire_coroner_budget_hook(db: Any, task_id: "UUID") -> None:
        ...
        """
        from robofleet.services.coroner_engine import get_coroner_engine
    ...
    async def _notify_agent_stranded(
        ...
        try:
            ...
            from robofleet.services.repositories.query_helpers import get_agent_by_role
            from robofleet.utils.converters import require_uuid
    ...
    async def _strategy_engine_cycle(self, state: _StrategyLoopState) -> None:
        ...
        from robofleet.services.strategy_engine import get_strategy_engine
    ...
    async def _self_heal_loop(self) -> None:
        ...
        if not settings.self_heal_enabled:
            return
        from robofleet.db import get_db_context
        from robofleet.services.self_heal_engine import get_self_heal_engine
    ...
    async def _load_env_sync_set(self, db: Any) -> list[Any]:
        ...
        """
        from robofleet.models.env_branches import ladder_pairs
        from robofleet.services.project import get_project_service
    ...
    async def _run_release_manager_cycle(self) -> None:
        ...
        from robofleet.db import get_db_context
        from robofleet.services.release_manager_engine import get_release_manager_engine
    ...
    async def _run_x_mentions_cycle(self) -> None:
        ...
        from robofleet.db import get_db_context
        from robofleet.services.x_engine import get_x_engine
    ...
    async def _run_vault_intake_cycle(self) -> None:
        ...
        from robofleet.db import get_db_context
        from robofleet.services.vault_intake_engine import get_vault_intake_engine
    ...
    async def _vault_janitor_loop(self) -> None:
        ...
        cadence (see ``robofleet.services.vault_janitor``).

        Dormant unless ``obsidian_vault_enabled`` — the umbrella flag.
        """
        if not settings.obsidian_vault_enabled:
            return
        from robofleet.services.vault_janitor import JANITOR_LOOP_INTERVAL_SECONDS
    ...
    async def _run_vault_janitor_cycle(self) -> None:
        ...
        from robofleet.db import get_db_context
        from robofleet.services.vault_janitor import get_vault_janitor
    ...
    async def _run_vault_kb_cycle(self) -> None:
        ...
        from robofleet.db import get_db_context
        from robofleet.services.vault_kb_engine import get_vault_kb_engine
    ...
    async def _run_telegram_poll_cycle(self) -> None:
        ...
        from robofleet.db import get_db_context
        from robofleet.services.telegram_inbound import get_telegram_inbound_engine
    ...
    async def _render_both_cuts(
        ...
        from robofleet.db import get_db_context
        from robofleet.services.project import get_project_service
        from robofleet.services.video_renderer_client import get_video_renderer
    ...
    def _repo_key(git_url: str) -> str:
        ...
        Delegates to :func:`robofleet.utils.converters.repo_key` so the dedupe
        queries and the poll-set collapse share one source of truth (#1267).
        """
        from robofleet.utils.converters import repo_key
    ...
    async def _park_gemini_auth_unavailable(self, agent_id: str, instance: Any) -> None:
        ...
        ``robofleet.llm.providers.gemini``'s module docstring for why none is
    ...
    async def _park_kimi_auth_unavailable(self, agent_id: str, instance: Any) -> None:
        ...
        robofleet.llm.providers.kimi), refreshed IN-PROCESS by the CLI itself
...
"""
    ...
    async def _refresh_grok_auth(self) -> None:
        ...
        try:
            from robofleet.llm.providers import grok_auth
            from robofleet.llm.providers.grok import GROK_AUTH_HOST_PATH
    ...
    async def _refresh_codex_auth(self) -> None:
        ...
        try:
            from robofleet.llm.providers import codex_auth
            from robofleet.llm.providers.codex import CODEX_AUTH_HOST_PATH
    ...
    async def _heal_stale_agent_tokens(self) -> int:
        ...
        """
        from robofleet.agents_config import _auth_secret, verify_agent_token
    ...
    async def _is_paused(self, scope: "PauseScope") -> bool:
        ...
        from robofleet.db.base import get_session_factory
        from robofleet.services.maintenance_pause import is_paused
    ...
    async def _pest_control_evidence_context(self) -> str:
        ...
        try:
            from robofleet.db import get_db_context
            from robofleet.services.pest_control_engine import get_pest_control_engine
    ...
    async def _scales_evidence_context(self) -> str:
        ...
        try:
            from robofleet.db import get_db_context
            from robofleet.services.scales_engine import get_scales_engine
    ...
    async def _sentinel_evidence_context(self) -> str:
        ...
        try:
            from robofleet.db import get_db_context
            from robofleet.services.sentinel_engine import get_sentinel_engine
    ...
    async def _megaphone_digest_context(self) -> str:
        ...
        try:
            from robofleet.db import get_db_context
            from robofleet.services.megaphone_engine import get_megaphone_engine
    ...
    async def _librarian_mining_context(self) -> str:
        ...
        try:
            from robofleet.db import get_db_context
            from robofleet.services.librarian_engine import get_librarian_engine
    ...
    async def _compose_parked_intake_redraft(
        ...
        from robofleet.foundation.policy.batch import is_batch_umbrella
        from robofleet.services.journal import get_journal_service
...
"""
    ...
    async def _check_sla_for_task(
        ...
        from robofleet.enforcement.task_lifecycle import sla_seconds_for
    ...
    async def _detect_sla_exceeded(self, client: httpx.AsyncClient) -> None:
        ...
        """
        from robofleet.enforcement.task_lifecycle import ROLE_STATE_SLA_KEYS
...
2. Compare inventories against each other, citing `file:line` for every
   claimed gap:
   a. API routes (robofleet/api/routes/) with no panel surface that exposes
