...
def _resolve_log_dir() -> Path | None:
    ...
    Order of precedence:
    1. $ROBOFLEET_LOG_DIR (explicit override).
    2. /data/logs — standard mount from docker-compose inside containers.
    3. ${ROBOFLEET_DATA_DIR:-./data}/logs — host-side fallback. This is the SAME
    ...
    """
    override = os.environ.get("ROBOFLEET_LOG_DIR")
    if override:
        return Path(override)
    container_path = Path("/data/logs")
    if container_path.is_dir() or container_path.parent.is_dir():
        return container_path
    return Path(os.environ.get("ROBOFLEET_DATA_DIR", "./data")) / "logs"
