"""Spackle engine: originate ONE held exploration cycle, deduped, never
authors content itself, and only against an opted-in project.

Mirrors test_pest_control_engine.py, minus the evidence_context tests (spec
...
async def _seed(session: AsyncSession, *, opted_in: bool = True) -> ProjectTable:
    ...
    project = ProjectTable(
        ...
        is_active=True,
        board_programs=["spackle"] if opted_in else None,
    )
    session.add(project)
    await session.flush()
    return project
...
def _arm(session: AsyncSession) -> None:
    session.add(SystemSettingTable(key="board_program.spackle.enabled", value="true"))


@pytest.mark.asyncio
async def test_disabled_creates_no_cycle(db_session: AsyncSession) -> None:
    await _seed(db_session)
    engine = SpackleEngine(db_session)
    assert await engine.run_cycle() is None
    assert await get_task_service(db_session).list_open_spackle_cycles() == []
...
async def test_enabled_originates_held_exploration_task(
    ...
    open_cycles = await get_task_service(db_session).list_open_spackle_cycles()
    ...
    assert cycle.source == SPACKLE_SOURCE
    assert cycle.project_id == project.id
    assert "Spackle" in cycle.title
...
async def test_dedupe_one_open_cycle(db_session: AsyncSession) -> None:
    ...
    _arm(db_session)
    await db_session.flush()
    await SpackleEngine(db_session).run_cycle()
    second = await SpackleEngine(db_session).run_cycle()
    assert second is None
    assert len(await get_task_service(db_session).list_open_spackle_cycles()) == ONE
...
async def test_settings_store_false_creates_no_cycle(db_session: AsyncSession) -> None:
    await _seed(db_session)
    db_session.add(
        SystemSettingTable(key="board_program.spackle.enabled", value="false")
    )
    await db_session.flush()
    engine = SpackleEngine(db_session)
    assert await engine.run_cycle() is None
...
async def test_no_legacy_flag_backdoor(db_session: AsyncSession) -> None:
    ...
    for spackle to fall back to (unlike roadmap/x_feature)."""
    await _seed(db_session)
    await db_session.flush()
    engine = SpackleEngine(db_session)
    assert await engine.run_cycle() is None
...
async def test_multi_opted_projects_names_all_in_description(
    ...
    second = ProjectTable(
        ...
        created_by=SYSTEM_UUID,
        is_active=True,
        board_programs=["spackle"],
    )
    db_session.add(second)
    _arm(db_session)
    await db_session.flush()

    first_task = await SpackleEngine(db_session).run_cycle()
    ...
    second_task = await SpackleEngine(db_session).run_cycle()
...
async def test_rotation_keyed_by_own_source_not_pest_controls(
    ...
    not be treated as recently spackle-explored too (they are independent
    ...
    # invisible to spackle's own rotation.
    pest_task = TaskTable(
    ...
    # Both projects are never-explored under SPACKLE_SOURCE, so the tie
    # breaks by deterministic order (created-first == `project`), exactly as
    # if the pest_control row didn't exist.
    task = await SpackleEngine(db_session).run_cycle()
    assert task is not None
