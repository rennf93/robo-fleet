...
and the ``python -m robofleet.vault rebuild`` CLI — all need "task + parent +
...
so this module never needs ``robofleet.services.task`` / ``robofleet.services.project``
...
from robofleet.services.vault_writer import FindingRow, TaskLinkRef, TaskNoteData
