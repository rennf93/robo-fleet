...
- CELL-scope team resolution: the `Learning` dataclass has no `team` field, so `_fetch_notify_agents` resolves the author's team at notification time via a single `select(AgentTable.team).where(AgentTable.id == learning.agent_id)` lookup (learning.py:264-272). If the resolved team is None the method returns `[]` immediately — never falling through to the unfiltered base query (which would notify all non-human agents, the original over-notification bug). The base query's `_HUMAN_ONLY_ROLES` exclusion and self-exclusion (`AgentTable.id != learning.agent_id`) are inherited by the CELL branch; only the team predicate is added.
