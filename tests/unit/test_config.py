...
def test_local_llm_base_url_allow_external(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(
        "ROBOFLEET_LOCAL_LLM_BASE_URL",
        "https://us-central1-aiplatform.googleapis.com/v1",
    )
    monkeypatch.setenv("ROBOFLEET_LOCAL_LLM_BASE_URL_ALLOW_EXTERNAL", "true")
...
def test_gcp_settings_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ROBOFLEET_GCP_PROJECT_ID", "my-proj")
    monkeypatch.setenv("ROBOFLEET_GCP_REGION", "europe-west1")
...
def test_redis_url_uses_rediss_when_memorystore_tls(
    ...
    monkeypatch.setenv("ROBOFLEET_GCP_MEMORYSTORE_HOST", "10.0.0.5")
    monkeypatch.setenv("ROBOFLEET_GCP_MEMORYSTORE_TLS", "true")
    monkeypatch.setenv("ROBOFLEET_GCP_MEMORYSTORE_PORT", "6379")
    monkeypatch.setenv("ROBOFLEET_REDIS_PASSWORD", "pw")
...
def test_redis_url_plain_when_memorystore_tls_false(
    ...
    monkeypatch.setenv("ROBOFLEET_GCP_MEMORYSTORE_HOST", "10.0.0.5")
    monkeypatch.setenv("ROBOFLEET_GCP_MEMORYSTORE_TLS", "false")
    monkeypatch.setenv("ROBOFLEET_REDIS_PASSWORD", "pw")
...
def test_redis_url_uses_memorystore_host_over_redis_host(
    ...
    monkeypatch.setenv("ROBOFLEET_GCP_MEMORYSTORE_HOST", "memorystore.example")
    monkeypatch.setenv("ROBOFLEET_GCP_MEMORYSTORE_TLS", "true")
    monkeypatch.setenv("ROBOFLEET_REDIS_PASSWORD", "secret")
...
def test_workspaces_root_default_when_filestore_unset(
    ...
    monkeypatch.delenv("ROBOFLEET_GCP_FILESTORE_SHARE", raising=False)
