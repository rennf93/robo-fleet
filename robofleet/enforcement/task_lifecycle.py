"""Backwards-compatibility shim — view of robofleet.foundation.policy.lifecycle.

The canonical lifecycle / permissions tables live in
:mod:`robofleet.foundation.policy.lifecycle`. This module is a thin view
over that data for legacy callers that still import
``VALID_TRANSITIONS`` or ``ROLE_RESTRICTED_TRANSITIONS`` by name. New
code should import from :mod:`robofleet.foundation.policy.lifecycle`
directly.
...
# canonical source of truth in `robofleet.foundation.policy.lifecycle`).
#
...
# :data:`robofleet.config.settings`. The orchestrator's stuck-task sweep
...
# applies." Defaults live in :class:`robofleet.config.Settings` so they're
