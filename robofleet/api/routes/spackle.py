"""Spackle (Board Program) engine API — the CEO approves/rejects items
within a held gap-fill cycle. CEO-only throughout. Approving an item
materializes it as a BACKLOG task; nothing here starts it — normal PM
activation takes it from there. Mirrors ``robofleet.api.routes.pest_control``.
"""
...
from robofleet.api.schemas.spackle import (
    GapFillItemActionResponse,
    GapFillRejectRequest,
    SpackleCycleResponse,
)
from robofleet.api.utils.spackle import _require_ceo, _to_response
...
async def list_spackle_cycles(
    db: DbSession, agent: CurrentAgentContext
) -> list[SpackleCycleResponse]:
    """Every open spackle cycle already authored by the Product Owner.
...
@router.post(
    ...
    response_model=GapFillItemActionResponse,
)
@guard_deco.rate_limit(requests=30, window=60)
@guard_deco.block_clouds()
async def approve_gap_fill_item(
    task_id: UUID,
    item_id: str,
    db: DbSession,
    ...
    if result is None:
        raise HTTPException(
            ...
            detail="No such open spackle item",
        )
    await db.commit()
    return GapFillItemActionResponse(
...
async def reject_gap_fill_item(
    task_id: UUID,
    item_id: str,
    data: GapFillRejectRequest,
    db: DbSession,
    agent: CurrentAgentContext,
) -> GapFillItemActionResponse:
