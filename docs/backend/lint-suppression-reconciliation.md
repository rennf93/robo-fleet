...
Sentinel's `no_lint_suppressions=32` figure was a live tally taken by the conventions scanner at report time — a count, not a persisted per-item ledger. `robofleet_git_log`/`robofleet_git_diff` (real read-only git tools available to a developer role) timed out on every attempt during this reconciliation session — a known, tracked platform-wide degradation, backend task `62845be1` — so most of the individual pre-cleanup file:line provenance for suppressions already removed by earlier rounds of this same audit could not be reconstructed from a diff read; five sites were instead recovered from the earlier rounds' own task-completion journals (see the reconciliation table below). What *is* checkable today is an exhaustive re-scan against the standard's own scope (`robofleet/` + `panel/`, excluding `tests/`/`docs/`/`node_modules` — the same exclusion the conventions standard applies everywhere else), run as:

```bash
grep -rn '# noqa\|# type: ignore' --include='*.py' robofleet/
...
|---|------|------|-------------|
| 1 | `robofleet/api/routes/prompter_live.py:242` | `ARG001` (unused `session_id`) | **Waived** — `preview_live_batch`'s `session_id` is unused (the endpoint is a pure precompute); FastAPI only binds path params a handler declares, so dropping it would not break routing. Kept for route symmetry with every sibling `/live/{session_id}/*` handler in the file. `.robofleet/conventions.yml`'s waiver reason was corrected by this task (it previously and incorrectly claimed dropping the param "breaks routing"). |
| 2 | `robofleet/foundation/policy/lifecycle.py` (the `from robofleet.foundation._validate_lifecycle import (` line) | `E402` (import not at top) | **Waived** — `_run_all_lifecycle_validators`'s import sits below other module code deliberately: `robofleet.foundation.__init__` eagerly imports `_validate`, and importing `_validate_lifecycle` at the top of this file would create a real circular import. Genuine constraint, unchanged by this task. |

### Still present, framework-exempt by standing policy (9)

Auto-allowed under `robofleet/conventions/hygiene.py`'s `_ALLOWED_SUPPRESSION_CODES` (`TC001`/`TC002`/`TC003`, pydantic `prop-decorator`) — these were never `no_lint_suppressions` findings under the scanner's own logic (the codes are sanctioned framework escape hatches, not silenced debt), listed here only so the census below is exhaustive and auditable.

| # | Site | Code |
|---|------|------|
| 3 | `robofleet/api/schemas/prompter_live.py:10` | `TC003` |
| 4 | `robofleet/api/schemas/provider.py:15` | `TC003` |
| 5 | `robofleet/api/schemas/provider.py:17` | `TC003` |
| 6 | `robofleet/api/schemas/provider.py:21` | `TC001` |
| 7 | `robofleet/config.py:101` | `prop-decorator` |
| 8 | `robofleet/config.py:158` | `prop-decorator` |
| 9 | `robofleet/config.py:167` | `prop-decorator` |
| 10 | `robofleet/config.py:184` | `prop-decorator` |
| 11 | `robofleet/config.py:214` | `prop-decorator` |
...
| 16 | `robofleet/services/permissions.py` | `F401`/`E402` | **Fixed at source** — PR #734 (task `da5a1985`); unused re-export resolved via a top-level import + `__all__` (the `__all__` list was later widened by this round's sibling task `e339d9bb`, an unrelated follow-up — no marker was re-added). |
| 17 | `robofleet/logging.py` | `E402` | **Fixed at source** — PR #734 (task `da5a1985`); import reordered to the top of the module. |
| 18 | `robofleet/services/kanban.py` | `E501` | **Fixed at source** — PR #734 (task `da5a1985`); duplicated over-length title lines deduplicated via a shared `_board_title()` helper. |
| 19-32 | — | **Residual — provenance not individually recoverable this session.** The exhaustive re-scan above confirms zero markers beyond the 18 named in this document remain anywhere in `robofleet/` or `panel/` today, so these 14 were genuinely removed at some point across the audit's two merged rounds or ordinary refactors on this lineage, but their specific pre-cleanup file:line history could not be reconstructed — neither round's own completion journal enumerates more sites than the 6 fixed-at-source (5 unique here, plus `orchestrator.py` already itemized as row 13) + 1 (`journals-view.tsx`, row 12) captured across this document, and the git tooling needed to inspect the actual historical diffs timed out on every attempt. This is the one honest residual disposition for this batch — not "outside this scan's current surface" and not "not available to a developer role," both of which were inaccurate framings in an earlier draft. |

**Running total: 2 + 9 + 2 + 5 + 14 = 32.**

## God_class correlation verdict

**Verdict: not causal.** Of the 13 suppression sites this audit can point to concrete evidence for (the 2 waived + 9 exempt + 2 fixed-at-source rows above), exactly **one** sat inside a god_class-flagged file: `robofleet/runtime/orchestrator.py`'s `AgentOrchestrator` class, which has ~492 methods — wildly over the `god_class` rule's 15-method threshold (`robofleet/conventions/modularity.py`). Every other known site sits in a normal-sized file or a class nowhere near that threshold:

- `robofleet/config.py`'s `Settings` class: 10 methods (under 15 — not god_class).
- `robofleet/foundation/policy/lifecycle.py`'s `Context` class and sibling dataclasses (`Decision`, `Precondition`, `ActionSpec`, `IntentSpec`, `StatusTransition`): 0 methods each — plain data holders, not god_class.
- `robofleet/api/schemas/prompter_live.py` / `provider.py`: Pydantic schema classes with 0 methods each — not god_class.
- `robofleet/api/routes/prompter_live.py`: route functions, no class at all.
