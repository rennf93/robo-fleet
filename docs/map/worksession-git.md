# RoboFleet Slice Map — `worksession-git`

Scope key: `worksession-git` Repo root: `/Users/renzof/Documents/GitHub/ZZZ/robofleet-master/robofleet` Files in scope:
- `robofleet/services/work_session.py`
- `robofleet/services/git.py`
- `robofleet/templates/git/` (`__init__.py`, `branch.py`, `commit.py`, `constants.py`, `pr_internal.py`, `pr_root.py`)
- `robofleet/services/forge/` (`__init__.py`, `base.py`, `github.py`, `gitea.py`, `gitlab.py`, `registry.py`, `router.py`, `shaping.py`)
- `robofleet/foundation/policy/forge.py`
- `robofleet/foundation/policy/pr_labels.py`

## Purpose

This slice is the git substrate every delivery agent works on. `GitService` runs all git subprocesses (status/commit/branch/push/rebase), mints branches + commit messages + PR bodies from templates, and drives the REST API for PR create/merge/close — now routed through a **multi-forge layer** (`robofleet/services/forge/`) instead of talking to GitHub directly. `WorkSessionService` persists the per-claim row that links an agent to a task's branch/commits/PR and enforces the single-active-per-task invariant. The `robofleet/templates/git/` package is the pure rendering layer for branch names, commit messages, and internal/root PR bodies. `robofleet/foundation/policy/pr_labels.py` derives the org-structure label vocabulary (`to master`/`to slave`, `root`, `MegaTask`, layer labels) applied at PR-open. Together they are the boundary between the task lifecycle and the actual git history on disk + the configured forge (GitHub, Gitea, or GitLab).

## Files

| Path | Role | approx LOC |
|------|------|------------|
| `robofleet/services/work_session.py` | WorkSession CRUD, commit/file tracking, PR-lifecycle record, single-active invariant | 685 |
| `robofleet/services/git.py` | Git subprocess execution, branch/commit/PR/rebase/merge/sync, GitHub REST API, conventions validator runner | 4596 |
| `robofleet/templates/git/__init__.py` | Package re-exports for branch/commit/PR templates | 48 |
| `robofleet/templates/git/branch.py` | Hierarchical branch name builder + root-task resolver | 131 |
| `robofleet/templates/git/constants.py` | `BRANCH_TYPES`, `COMMIT_TYPES`, `MAX_TASK_DEPTH`, length constants | 52 |
| `robofleet/templates/git/commit.py` | `CommitContext` + `build_commit_message` (traceability links) | 114 |
| `robofleet/templates/git/pr_internal.py` | Internal (subtask→parent) PR title/body builder | 140 |
| `robofleet/templates/git/pr_root.py` | Root (→master, CEO-level) PR title/body builder with task tree | 245 |
| `robofleet/services/forge/__init__.py` | Package re-exports (`ForgeRouter`, `GitProvider`, `GitHubProvider`, `GiteaProvider`, `GitLabProvider`, `RepoRef`, `provider_for`, `register_project_forge`) | 32 |
| `robofleet/services/forge/base.py` | Pure contract: `RepoRef` dataclass + the `GitProvider` ABC (~20 abstract methods every forge implements) | 208 |
| `robofleet/services/forge/github.py` | GitHub.com/GHE REST transport — the pre-existing inline `httpx` logic pulled out unchanged; owns the sole retry policy in the package (`list_ci_runs`) | 432 |
| `robofleet/services/forge/gitea.py` | Self-hosted Gitea REST transport, adapting the wire contract back into GitHub shapes | 471 |
| `robofleet/services/forge/gitlab.py` | GitLab REST v4 transport — heaviest adaptation (MR `iid`, diff reassembly, pipelines) | 679 |
| `robofleet/services/forge/registry.py` | Host↔provider(+scheme) map + `provider_for(project)` resolution, self-healing per-process | 104 |
| `robofleet/services/forge/router.py` | `ForgeRouter` — implements `GitProvider` by picking a transport per call from `RepoRef.host` | 195 |
| `robofleet/services/forge/shaping.py` | `ShapedResponse` — an `httpx.Response`-compatible stand-in a non-GitHub provider returns when it must synthesize a status the wire call didn't produce (e.g. a shaped 501 for `merge_branch`) | 51 |
| `robofleet/foundation/policy/forge.py` | Pure host/provider detection (`extract_host`, `detect_provider`) + registration-time validation (`validate_project_forge`) | 99 |
| `robofleet/foundation/policy/pr_labels.py` | `derive_pr_labels` — the org-structure label vocabulary (`to <base_branch>`, `root`, `MegaTask`, layer labels) applied at every PR-open site | ~90 |
...
| `GitService._run_conventions_validator` | method | git.py:4408 | Subprocess `python -m robofleet.conventions` with 120s cap |
| `GitService.open_conventions_pr` | method | git.py:4456 | Scaffold `.robofleet/conventions.yml` on a branch + open PR |
...
| `GitLabProvider` | class | forge/gitlab.py:85 | GitLab REST v4 transport — the most semantically divergent: MR `iid`→`number`, `source_branch`/`target_branch`→`head.ref`/`base.ref`; `get_pr_diff` reassembles a unified-diff string from up to 3 pages of per-file JSON diffs (no raw-diff media type); `post_review` routes APPROVE to `/approve` and everything else to a plain note (no request-changes verb exists); `request_reviewers` synthetic-skips (needs numeric user ids RoboFleet doesn't store); `create_org_repo` resolves a group path to a numeric namespace id, falling back to the token's personal namespace on 404 |
...
Internal:
- `robofleet.config.settings` (timeouts, URLs, workspace root, auto_clone)
- `robofleet.exceptions` (`GitCommandError`, `GitError`, `GitTimeoutError`, `MergeConflictError`)
- `robofleet.foundation.policy.lifecycle` (role/intent parity)
- `robofleet.models.base` (`AgentRole`, `TaskStatus`)
- `robofleet.models.work_session` (`WorkSessionCreate/Update/Status`)
- `robofleet.db.tables` (`ProjectTable`, `TaskTable`, `WorkSessionTable`)
- `robofleet.services.base` (`BaseService`, `NotFoundError`, `ConflictError`, `ValidationError`, `UnauthorizedError`, `ServiceError`)
- `robofleet.services.project` / `robofleet.services.task` / `robofleet.services.workspace` (composed for clone/workspace/branch resolution)
- `robofleet.services.gateway.quality_gate` (`GateResult`, `run_quality_commands`)
- `robofleet.templates.git` (all template builders)
- `robofleet.utils.converters.require_uuid`, `robofleet.utils.crypto.EncryptionError`
- `robofleet.api.schemas.git` (TYPE_CHECKING only — runtime duck-typed)
...
|----------------|--------|----------|
| `ROBOFLEET_GIT_EXECUTOR_WORKERS` (env, default 16) | `os.environ` at git.py:117 | Dedicated git subprocess pool size |
...
- **CLAUDE.md says** `ROBOFLEET_WORKSPACE_CLONE_TIMEOUT=300` is a WorkspaceService config; not referenced in this slice (lives in `workspace.py`). No drift in scope.
