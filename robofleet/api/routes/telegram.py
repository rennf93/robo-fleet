...
middleware live inline in ``robofleet.api.app`` (mirroring
``robofleet.api.auth.routes.mount_cloud_auth``'s conditional-mount pattern; app
...
from robofleet.api.auth.backend import cookie_transport, get_jwt_strategy
from robofleet.api.deps import CurrentAgentContext, DbSession, require_ceo_role
from robofleet.api.schemas.telegram import (
    TelegramCredentialsSetRequest,
    TelegramCredentialsStatus,
    TelegramTodayResponse,
    TelegramWebAppAuthRequest,
)
from robofleet.api.utils.telegram import _require_ceo
...
# ``robofleet.api.app`` (see the module docstring above).
# ==========================================================================
...
async def webapp_auth(
    ...
    # robofleet.api.auth.seed.ensure_seed_user (ordered by the primary key's
