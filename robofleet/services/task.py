...
from robofleet.enforcement import (
    GitContext,
    TaskLifecycleError,
    TaskOwnershipError,
...
from robofleet.events import Event, EventType, get_event_bus
...
from robofleet.foundation.policy.content import Finding, Severity, markers
...
from robofleet.models.env_branches import effective_environments, head_branch
from robofleet.models.permissions import AgentContext, TaskAction
from robofleet.models.task import TaskCreateRequest
from robofleet.models.work_session import WorkSessionCreate
...
from robofleet.services.content_notes import apply_structured_note
from robofleet.services.repositories.review_findings import ReviewFindingsRepository
from robofleet.services.work_session import WorkSessionService
from robofleet.utils.converters import repo_key, require_uuid, to_python_uuid
...
def _repo_key_expr(column: Any) -> Any:
    """SQL mirror of :func:`robofleet.utils.converters.repo_key`: lower, strip a
...
class StalledTaskEntry:
    ...
    Backs the read endpoint in ``robofleet/api/routes`` — the route only
...
# X_CAMPAIGN_SOURCE in robofleet.services.x_engine).
...
async def _evict_stranded_agent(task_id: UUID, prior_holder: UUID) -> None:
    ...
    """
    try:
        from robofleet.api.deps import get_orchestrator_or_none
        from robofleet.services.agent import get_agent_service
...
# (``robofleet/eval/``) replays through the real delivery lifecycle to score a
...
class TaskService(BaseService):
    ...
    def _validate_and_set_status(
        ...
        # don't break.
        try:
            from robofleet.api.deps import get_orchestrator
    ...
    def _touch_vault_frontmatter(
        ...
        if not settings.obsidian_vault_enabled:
            return
        try:
            from robofleet.services.vault_writer import get_vault_writer
    ...
    def _enforce_no_pm_code_on_create(self, req: TaskCreateRequest) -> None:
        ...
        if not req.assigned_to:
            return
        from robofleet.foundation.identity import role_for_uuid_or_none
    ...
    async def project_default_branch_for_task(self, task: Any) -> str | None:
        ...
        (:func:`robofleet.services.gateway.merge_chain.resolve_parent_branch`) when
    ...
    async def _merge_one_dependency(
        ...
        from robofleet.services.gateway.merge_chain import resolve_parent_branch
    ...
    async def mark_stalled(self, task_id: UUID, reason: str) -> None:
        """Set the durable stalled marker (see `robofleet.models.base.StalledReason`).
    ...
    def _resolve_sequence_blocker(
        ...
        if is_batch_root_subtask(
            ...
            return candidates[0]
        from robofleet.services.sequencing import sequence_blocker_id
    ...
    async def _inject_proactive_context(self, task: TaskTable, agent_id: UUID) -> None:
        ...
        from robofleet.db.base import get_session_factory
        from robofleet.services.proactive import get_proactive_service
    ...
    async def _extract_completion_learnings(
        ...
        from robofleet.services.learning import get_learning_service
    ...
    async def _record_one_completion_learning(
        ...
        from robofleet.services.learning import RecordLearningParams
        ...
        try:
            ...
            from robofleet.services.rag_index_failures import persist_failure
    ...
    async def _index_code_changes_background(
        ...
        from robofleet.services.optimal import get_optimal_service
    ...
    async def _index_decisions_background(
        ...
        from robofleet.models.optimal import IndexDecisionParams
        from robofleet.services.optimal import get_optimal_service
    ...
    def _resolve_doc_abspath(rel_path: str) -> str:
        ...
        from robofleet.services.docs import DOCS_BASE_PATH
    ...
    async def _index_qa_review_background(
        ...
        from robofleet.models.optimal import IndexReviewParams
        from robofleet.services.optimal import get_optimal_service
    ...
    async def _index_qa_errors_background(
        ...
        from robofleet.models.optimal import IndexErrorParams
        from robofleet.services.optimal import get_optimal_service
    ...
    async def _abandon_work_session_best_effort(
        ...
        """
        try:
            from robofleet.services.work_session import (
                WorkSessionService,
            )
    ...
    async def _maybe_advance_to_pm_review(self, task: TaskTable, task_id: UUID) -> None:
        ...
        """
        from robofleet.enforcement.task_lifecycle import check_parallel_completion
    ...
    async def ceo_approve(
        ...
        try:
            from robofleet.services.gateway.choreographer.findings import (
    ...
    async def wire_sibling_collision_dag(self, parent_task_id: UUID) -> None:
        ...
        """
        from robofleet.services.sequencing import dev_task_collision_edges
    ...
    async def wire_cell_task_wave_chain(self, cell_task_id: UUID) -> None:
        ...
        """
        from robofleet.services.sequencing import cell_task_wave_chain_depends_on
    ...
    async def wire_by_osmosis_edge(self, dev_task_id: UUID) -> None:
        ...
        """
        from robofleet.services.sequencing import by_osmosis_tail_dev_tasks
    ...
    async def substitute_task_for_agent(
        ...
        from robofleet.models.base import SubstituteReason
    ...
    async def escalate(
        ...
        """
        from robofleet.agents_config import get_escalation_target
...
async def resolve_pm_for_substitute(
    ...
    """
    from robofleet.agents_config import get_pm_for_agent, get_pm_for_team
